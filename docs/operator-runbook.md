...
- 品牌、视觉和 accessibility 约束以 `PRODUCT.md` 为准；产品取舍以 `docs/product-charter.md` 为准，现有能力与共享回归范围以 `docs/product-map.md` 为准，历史逃逸问题以 `docs/regression-corpus.md` 为准，代码规范以 `docs/code-standards.md` 为准，结构和数据边界以 `docs/architecture.md` 为准，待处理债务以 `docs/code-cleanup-map.md` 为准。
...
- NodeSeek、linux.do 和妖火 Cookie，以及 NodeImage API Key、保存的账号密码和代理配置不进入备份 JSON。
...
- More 页只有一个 `账号中心`：统一显示三个账号来源的状态和当前身份，并提供公共 `刷新账号状态`；自动填入服务这三个站点，NodeImage 保持独立服务入口；App 内没有伪匿名测试入口，代理、诊断、备份和外观保持独立。
...
- 账号状态刷新由 `src/features/account/useAccountStatusController.ts` 提供，备份 I/O 由 `src/features/more/useBackupStatusController.ts` 提供；普通启动只恢复本机账号终态，不发 Account 请求。进入 More 本身不刷新，只有用户点击公共“刷新账号状态”才并行核对已启用来源。
...
- 涉及首页、搜索、收藏、用户页长列表或详情图片 cache 时，运行相关体验 / 性能测试和 `npm run typecheck`；图片 cache 另执行 `src/platform/media/originalImageLoading.test.ts`、`src/platform/media/compatibleImageSources.test.ts`、`src/features/topic/rendering/useHtmlRenderingController.test.tsx` 与 `npm run check:react`。
- 涉及首页来源、分类、单站排序或分页缓存时，至少运行 `npm test -- src/domain/forum/feed.test.ts src/domain/forum/feedOptions.test.ts src/sources/feedRead.test.ts`、`src/sources/searchRead.test.ts`、`src/sources/sourceTopicRead.test.ts`、`src/sources/sourceUserRead.test.ts`、`src/sources/sourceAccountRead.test.ts tests/integration/source-read-contracts/` 和 `npm run typecheck`，并在模拟器检查对应单站筛选。
- 涉及登录、验证、Cookie、写操作、详情返回或来源解析时，运行相关来源 / 安全 / 体验测试和 `npm run typecheck`。
- 涉及来源 gateway 时，至少运行 `npm test -- src/sources/feedRead.test.ts`、`src/sources/searchRead.test.ts`、`src/sources/sourceTopicRead.test.ts`、`src/sources/sourceUserRead.test.ts`、`src/sources/sourceAccountRead.test.ts tests/integration/source-read-contracts/ src/sources/readGateway.test.ts src/sources/readGatewayContract.test.ts` 和 `npm run typecheck`。
- 涉及账号区时，至少运行账号中心、会话、凭据仓库、登录表单 adapter、NodeImage 和来源测试及 `npm run typecheck`；统一 UI 位于 `src/features/more/components/AccountCenterPanel.tsx`，视图规则位于 `src/features/more/accountCenter.ts`，三个账号来源的凭据和填入边界位于 `src/platform/storage/credentialVault.ts`、`src/domain/session/loginFormAdapters.ts`。
- 改 `plugins/withSecureRandomModule.js` 或 `app.json` 的 SecureRandom plugin 后，至少运行 Expo config、clean prebuild、Release Kotlin 编译和 Native 单元测试，确认生成的最小 Native 模块、MainApplication 注册与 NodeImage 调用链可编译；正式发布仍只在明确发布任务中运行 `npm run release:android`。
- 涉及服务器代理时，至少运行 `npm test -- src/platform/network/networkProxy.test.ts src/platform/update/useAppUpdateRuntime.test.ts tests/tooling/release-packaging.test.ts`、`npm run test:ui -- tests/ui/more/network-proxy-controller.test.tsx tests/ui/more/network-proxy-modal.test.tsx tests/ui/account/hidden-browser-host.test.tsx tests/ui/account/account-host.test.tsx tests/ui/account/account-site-panels.test.tsx tests/ui/topic/topic-image-loading.test.tsx tests/ui/app/app-runtime-startup.test.tsx` 和 `npm run typecheck`。改 `plugins/withNetworkProxyModule.js` 后必须 fresh `npx expo prebuild --platform android --clean --no-install`，再从 `android/` 运行 `gradlew :app:testReleaseUnitTest :app:compileReleaseKotlin --no-daemon`；正式发布仍运行 `npm run release:android`。
- 涉及消息中心、`expo-notifications`、TaskManager 或后台调度时，至少运行 `src/sources/discourseNotifications.test.ts`、`src/sources/nodeseek/notifications.test.ts`、`src/sources/yaohuo/notifications.test.ts`、`src/sources/notificationGateway.test.ts`、`src/platform/notifications/notificationStore.test.ts`、`src/platform/notifications/notificationWorker.test.ts`、`src/platform/notifications/notificationSystem.test.ts`、`src/app/AppNavigator.test.ts`、`tests/ui/notifications/notifications-screen.test.tsx`、`tests/ui/notifications/notifications-runtime.test.tsx`、`tests/ui/notifications/notifications-route.test.tsx`、`tests/ui/shared/accessibility-basics.test.tsx` 和 More、账号、代理 fail-closed、release packaging 相关测试，再执行 `npm run typecheck`。这些测试必须覆盖 adapter auth/CF/畸形响应、官方字段/稳定 ID 与歧义行保守丢弃、NodeSeek comment ID 有界逐页、妖火目标缺失失败、聚合失败来源定向重试的 expected identity/cancel/末页 cursor、真正 unknown 或登录 surface barrier 时缓存不可见与确认退出清 aggregate、foreground handler 与共用 worker、worker 沿 cursor 跨页扫描 60 条、投递失败回滚、记录后开关/换号竞态、硬件返回和 48dp/读屏动作。随后 fresh Expo prebuild，检查生成的通知 permission/channel/icon、foreground handler 与顶层 task 注册，并运行 `android\gradlew.bat :app:testReleaseUnitTest :app:compileReleaseKotlin --no-daemon`。匹配 APK 的 `tests/device/notifications-readonly.ad` 只读覆盖总览/三站 outcome、未读开关、设置三站和系统返回；前台/后台新消息摘要、消息详情、下拉刷新、字号/TalkBack 需匹配 APK 手动/只读 Live，真实已读需另行授权，未执行项记 `NOT_VERIFIED`。
- 涉及主题或详情页拆分时，至少运行 `npm test -- tests/integration/style-ownership.test.ts src/features/topic/model/topicDerivedData.test.ts src/domain/forum/topicContentSplit.test.ts src/domain/forum/topicContentHtml.test.ts src/domain/forum/topicListItemState.test.ts src/features/topic/useTopicSessionController.test.ts`、`npm run test:ui -- --runInBand tests/ui/app/app-navigator.test.tsx tests/ui/topic/topic-session-controller.test.tsx` 和 `npm run typecheck`，并在模拟器上验证外观设置与详情页打开 / 返回。
...
1. 用 `src/domain/forum/links.ts` 的 `parseForumTopicLink` 规则解析来源和主题 id，并使用解析结果中的规范化 URL。
...
- 发布候选先用当前开发包在主设备跑七条普通 Replay，并在隔离 AVD 跑一条真实未登录 Replay；再依次通过 `npm run verify`、正式签名构建与 signer 校验，并由同代码的开发签名 x86_64 Release APK 在唯一登录态设备上完成 APK sanity 与七条普通 Replay；最后按授权执行 `full` Agent Live。
