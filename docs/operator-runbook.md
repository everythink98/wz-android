...
- 涉及首页来源、分类、单站排序或分页缓存时，至少运行 `npm test -- src/domain/forum/feed.test.ts src/domain/forum/feedOptions.test.ts src/sources/readOrchestration.test.ts tests/integration/source-read-contracts.test.ts` 和 `npm run typecheck`，并在模拟器检查对应单站筛选。
- 涉及登录、验证、Cookie、写操作、详情返回或来源解析时，运行相关来源 / 安全 / 体验测试和 `npm run typecheck`。
- 涉及来源 gateway 时，至少运行 `npm test -- src/sources/readOrchestration.test.ts tests/integration/source-read-contracts.test.ts src/sources/readGateway.test.ts src/sources/readGatewayContract.test.ts` 和 `npm run typecheck`。
