...
- 涉及账号区时，至少运行账号中心、会话、凭据仓库、登录表单 adapter、小隐寺 Device Code 和来源测试及 `npm run typecheck`；统一 UI 位于 `src/features/more/components/AccountCenterPanel.tsx`，视图规则位于 `src/features/more/accountCenter.ts`，原三站凭据和填入边界位于 `src/platform/storage/credentialVault.ts`、`src/domain/session/loginFormAdapters.ts`，小隐寺授权边界位于 `src/sources/xiaoyinsi/auth.ts`。
