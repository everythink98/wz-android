...
| 必须保持的行为 | Pan 使用 `activeOffsetX([-10, 10])`、`failOffsetY([-10, 10])` 和单指；横向意图越阈值才接管，纵向越阈值立即失败并交还 FlashList。更新值按 `[0, contentWidth - viewportWidth]` clamp，松手以同范围 `withDecay`；route scope + `semanticId` 共享 offset，table segments 与重挂实例同步。code/table 保留文本选择、完整复制、查询高亮、terminal Tab 状态，并暴露无障碍增减滚动动作。 |
| 精确失败 oracle | `tests/ui/topic/topic-table-rendering.test.tsx` 的 `[REG-TOPIC-094]` 固定两种 renderer 使用相同阈值、被动 ScrollView、单指、clamp/decay 和无障碍动作；同表两个 segment 在一次 Pan 后都收到相同 offset，重挂后恢复。纵向失败路径不得改变横向位置。 |
