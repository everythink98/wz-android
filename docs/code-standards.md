...
| 路径 | 唯一职责 |
| --- | --- |
| `src/app/` | Expo bootstrap 之后的组合根、导航、全局 host 和无条件挂载的 App 级编排 |
...
- `scripts/check-architecture.mjs` 使用仓库已安装的 TypeScript AST 检查六类根目录、依赖矩阵、跨 feature/provider、barrel 和依赖环；规则失败应修正 ownership，不得用文件级豁免绕过。
