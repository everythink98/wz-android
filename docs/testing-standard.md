...
`REG-TOPIC-086` 补充逻辑节点连续性验收：compiler 必须用 52 行、约 191 个装饰元素的单个 `<pre>` 证明其先归一为一个 `codeBlock` semantic owner，而不是按原始 DOM node 数切碎；真正超限代码只按完整行分段。`details > callout > listItem > pre/table` 必须让每个 typed row 直接携带完整 `ancestorFrames`；内容不缺失、不重复、不乱序且每 row 继续满足硬预算。来源伪造的 `data-wz-*` 必须无效，输出不得生成 compiler binding 属性。renderer RNTL 必须证明 first/middle/last code segments 组成一个逻辑 frame、零双边框并以 `scope + semanticId` 同步/恢复横向 offset；table、details、callout、blockquote、list 与五站正文回归继续通过。
...
`REG-TOPIC-090` 补充 terminal report 全管线验收：来源集成测试必须从 NodeSeek magic tabs/ANSI 实际 HTML 经共享 sanitizer 进入 `compileForumContent()`，固定 report header、全部 tab、ANSI 前/背景色、图片和 report 外正文保序。compiler 必须固定总 report 超预算仍按 tab body 递归生成既有 typed rows、长 terminal code 按完整行分段且仅首段持有完整复制文本、单个不可拆 body 只局部 fail-close；opening、reply、signature、评论完整引用、主楼内嵌完整引用和 accepted preview/full 的模型入口都要覆盖。RNTL 必须贯通真实 reply model/filter/ReplyItem/TopicContentBlock，固定 stable item type、route-scoped active tab 在 cell 回收和筛选隐藏/恢复后保持、Topic identity 改变时重置，并验证 tab 无障碍、文本选择、查询高亮、复制成功/失败、原生横向滚动条/offset 与 ANSI 前/背景色。匹配 APK 只读直达 NodeSeek `post-812712-1`，再在同一 APK 回归 V2EX table、linux.do plain code/长图和 NodeSeek 2000 图片；单一多图片案例、局部 renderer 或 Smoke 不能记通过。
