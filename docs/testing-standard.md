...
`REG-TOPIC-062/063/067/068` 补充详情/回复标准：远端楼层定位采用锚点双向窗口。极大目标只请求目标窗口，随后用户触边才各请求一个相邻 cursor；不得逐页追赶、扩大 page-size 或补齐中间页。目标实体、当前页或前后 cursor 无法确认时保留原列表并明确失败。用户名与楼层是独立点击目标；定位前恢复“全部、空搜索”但保留当前 `ReplyOrder`，成功后滚动并短暂高亮。三种内容筛选固定在左侧 rail，右侧单选菜单显示并切换正序/倒序；顺序使用独立状态和 Query key，可与内容筛选、评论内查找组合，排序入口、菜单与边界文案随阅读字号一致缩放。来源 adapter 返回的顺序就是展示顺序，UI 不得反转部分集合。切换部分集合到倒序时先显示回复级 Loading，再分别验证 NodeSeek `postPageCount` / 严格 pager 尾页、Discourse stream 尾部 IDs 或妖火 `tofloor` resolved page；NodeSeek 详情没有明确总数时保持缺失，不额外抓末页计数、不展示已加载窗口大小，正序/倒序/双侧扩展均忽略旧 `replyCount`；Discourse hydration 在正倒序都必须返回与所选窗口完整、精确一致的 ID 集；妖火必须从主题页真实 `reply` / `tofloor` 链接取最大楼层，禁止把每页“更多回帖(N)”当回复总数，且须按原站 page 1 最新、page + 1 更早的方向转换 cursor；V2EX 只有实际条目数与权威计数严格相等时才允许复用完整集合。未确认边缘窗口、`parse_empty`、重复 cursor 或来源明确报告的计数竞态必须报错且不应用结果；计数竞态的显式重试只执行一次“刷新权威计数后从当前顺序 start 重建”，NodeSeek 不进入该恢复。最后一个 `next` cursor 结算后当次显示当前顺序的回复边界，后续触底不得再请求；向前插入必须保持可见位置，最后一个 previous cursor 插入时也不能提前关闭位置保持。最低自动证据覆盖五站 adapter、顺序 Query contract、Controller latest-command-wins、RNTL 切换/筛选/重试/路由恢复、边界终态与边缘映射，以及编辑/删除真实 `pageParam` 和新回复服务端尾窗刷新；匹配 revision/APK 的五站设备验收只读执行，不得发送回复或改变登录态。
