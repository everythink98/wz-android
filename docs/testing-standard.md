...
| 互动 / 写操作 | 未登录时不发送；原三站登录后请求带正确 Cookie / CSRF / sid，小隐寺只带独立 User API headers；投票、收藏、点赞、回复的目标不串站。linux.do 与小隐寺的 Discourse 点赞/书签共享 MutationCache 与精确 Query cache：先局部显示，失败恢复原状态，确认后同步活动 route snapshot；投票经服务器确认后局部更新且不整篇重载；Live 验收通过刷新或重进核对服务器状态。单个主题 403 先复核会话而不直接全局退出；复核失败或确认失效都必须保留原始动作错误并正常收口 Promise，且不得自动删除原站 Cookie。NodeSeek 投票读取失败时保留原始标记并诊断为 `partial`；提交前确认，确认后严格 `POST × 1 → GET × 1`，GET 失败不重投、不伪造票数 | `src/discourseActions.test.ts`、`src/discourseSourceActions.test.ts`、`src/discoursePermissions.test.ts`、`src/localSources.test.ts`、`src/xiaoyinsiActions.test.ts`、`src/xiaoyinsiActionClient.test.ts`、`src/nodeseekActions.test.ts`、`src/nodeseekActionClient.test.ts`、`tests/ui/topic-actions-controller.test.tsx`、`src/linuxdoActionClient.test.ts`、`src/yaohuoActions.test.ts`、`src/yaohuoActionClient.test.ts`、`src/topicActionState.test.ts` |
| 用户页 | 五站用户资料、头像、发帖数 / 回帖数、主题列表、回复列表、分页游标正确，已缓存下一 cursor 仍须展示；Profile Query 刚显示 next cursor 时立即分页也必须先 seed 对应 Infinite Query lane，并只请求该 cursor 一次；同一结构化 key 的观察者共享一个 transport 和 Query result；带 `parse_empty` 诊断的资料或列表页不得覆盖旧状态或推进 cursor；真实会话变化只清除对应来源旧资料、释放 Loading 并保留可重试身份；主题和回复的来源、分类、标题、作者、时间、楼层、摘要按原站支持范围显示；用户名和用户 ID 不混用 | `src/app/useUserController.test.ts`、`src/app/serverState.test.ts`、`src/forumApi.test.ts`、`src/localYaohuo.test.ts`、`src/localXiaoyinsi.test.ts`、`src/yaohuoApi.test.ts`、`src/screens/user/userScreenItems.test.ts`、`src/components/TopicCard.test.ts`、`src/userNavigation.test.ts`、`tests/ui/user-controller-session.test.tsx` |
...
| 登录 / 验证 / Cookie / Device Code / 凭据 | `SiteSessionState` 只承载凭据 transaction、登录弹层和验证/授权 workflow；账号中心四站远端 data/error/busy 由按站独立 Account Query 直接派生。单站读取失败保留该站上次可信 data，不阻断其他站；外部会话变化只 reset 目标站 active/disabled Query 与该站/`all` 私有缓存，查询自己确认失效则先提交 exact `expired` 再 seed 新 scope，见 `REG-ACCOUNT-016/019`。四站分别使用真实当前身份协议：只有 current user 或可信 self-account 结构算登录，只有协议明确匿名结果或准确游客结构算退出；Cookie 名/存在、旧 ID、Topic 作者、公开资料、普通业务文案、CF、网络错误、畸形 200 都不能证明身份。只要已有本站候选凭据，就不得因缺少某个写死 Cookie 名而跳过 current-session verifier。NodeSeek 只读当前页 `__config__.user`/self-account，linux.do 只读 `/session/current.json`，妖火只认 `wapindex.aspx?sid=-2` 可信本人导航，小隐寺只认 User API current session；资料补全失败不得推翻已证明身份，见 `REG-ACCOUNT-020/025/026`。被动 `cookie-loaded` 不携带身份结论，普通 Feed/Search/Topic/隐藏 WebView 不能降级可信身份；明确登录、失效、授权完成与手动清除只作用于目标站。原站 WebView Cookie 由网站与 Android `CookieManager` 持有：检测、刷新、过期、fallback 和普通失败只读映射，不得把 SecureStore 快照作为 WebView `Cookie` header 回灌，也不得删除原站 Cookie；只有用户明确点击该站“清除登录”才允许定向删除。隐藏 NodeSeek/linux.do WebView 挂载前等待共享 CookieManager，原生桥不得读私有 WebView 数据库。显式清除是 multi-store 事务，全部成功才发布 `cleared`；NodeSeek 保留 CF Cookie。WebView bridge 校验站点 HTTPS origin 与当前文档所有权。linux.do clearance 只是恢复候选；小隐寺 Device Code、Key/Client ID、重授权与撤销保持独立，普通复核失败不重复授权或清理。当前身份协议必须有官方源码/文档、站点实际调用或成熟客户端证据，测试 mock 不能创造 endpoint、状态码或清理权限。秘密不进日志/备份，页面区分匿名、验证中、授权中、失效、拒绝、过期与普通失败 | `tests/ui/account-status-controller.test.tsx`、`tests/ui/account-controller.test.tsx`、`src/app/useAccountCredentialController.test.ts`、`src/app/accountCredentialDiagnostics.test.ts`、`src/app/useVerificationController.test.ts`、`tests/ui/topic-actions-controller.test.tsx`、`src/siteSessionState.test.ts`、`src/app/sessionControllerHelpers.test.ts`、`src/app/serverState.test.ts`、`src/sources/sourceGatewayContract.test.ts`、`src/xiaoyinsiAuth.test.ts`、`tests/ui/feed-controller-xiaoyinsi.test.tsx`、`tests/ui/search-controller-ai.test.tsx`、`tests/ui/topic-session-controller.test.tsx`、`tests/ui/user-controller-session.test.tsx`、`tests/ui/xiaoyinsi-auth-controller.test.tsx`、`src/credentialVault.test.ts`、`src/loginFormAdapters.test.ts`、`src/screens/more/accountCenter.test.ts`、`src/nodeseekCookies.test.ts`、`src/yaohuoCookies.test.ts`、`src/yaohuoCookieBridge.test.ts`、`src/cookieCleanup.test.ts`、`src/appSecurity.test.ts` |
...
| 更多页 / 外观 / 更新 | 单一账号中心按 NodeSeek、linux.do、妖火、小隐寺排列且只显示一站详情；顶部区分待处理、四站网站登录和原三站自动填入数量；原三站服务入口均保留，小隐寺显示本人主页、等级及独立授权管理；进入 More 页不自动刷新；测试工具独立；代理密码遮蔽、完整连通性文案、备份、诊断、外观和更新行为保持独立 | `src/app/accountStatusHelpers.test.ts`、`src/screens/more/accountCenter.test.ts`、`tests/ui/account-center.test.tsx`、`tests/ui/more-screen.test.tsx`、`tests/ui/xiaoyinsi-auth-controller.test.tsx`、`tests/ui/network-proxy-modal.test.tsx`、`src/localXiaoyinsi.test.ts`、`src/siteSessionState.test.ts`、`src/xiaoyinsiAuth.test.ts`、`src/credentialVault.test.ts`、`src/loginFormAdapters.test.ts`、`src/networkProxy.test.ts`、`src/webViewProxyGuard.test.ts`、`src/appUpdate.test.ts`、`src/releasePackaging.test.ts` |
...
`REG-ACCOUNT-026` 补充 Cookie 所有权验收：账号检测、公共刷新、明确过期、fallback 和普通读写失败都不得删除或覆盖原站 WebView Cookie；原三站只有用户明确点击“清除登录”才拥有定向删除权限。隐藏 NodeSeek/linux.do WebView 必须从 Android 共享 CookieManager 读取当前原站会话，挂载前等待 `flush`，首跳只传 URL，公开请求 view 不得携带 App 快照 Cookie；原生桥源码必须只使用公开 CookieManager API。四站 Cookie 名只作候选摘要，不得直接判登录，也不得在存在候选时阻止真实 current-session 验证。最低测试覆盖清理命令不可达、隐藏 WebView props、CookieManager barrier、私有数据库源码禁用以及 NodeSeek/linux.do 的无已知登录 Cookie 名正向 current-user 响应。
...
`REG-ACCOUNT-028` 补充空凭据验收：NodeSeek 普通加载在 CookieManager/SecureStore 都为空时，只能返回 `undefined` 或发布不带身份结论的 `cookie-loaded`；不得发布 `verification-required`、`login-expired` 或 `cleared`。最低测试从 `logged-in`、`expired`、`verification-required` 三种可信状态执行被动空读取，要求身份、current user、最后确认时间和 Query scope 均不变；用户明确清除的 multi-store 事务仍必须在成功后发布 `cleared`。
...
`REG-WRITE-022` 补充写操作失效验收：NodeSeek typed `loginRequired` 与妖火 `reason: expired` 只在 credential generation 仍有效时发布目标站 `login-expired`；妖火 `reason: verification` 只发布 `verification-required`。明确失效必须通过现有 Session dispatcher 关闭 `canWrite`、清除 current user 并失效目标来源与 `all` 私有 Query scope，提示恰好一次且零 Cookie 清理；unknown、对象权限不足、普通失败和 stale generation 不改变身份。默认设备验收不得发送真实写操作，只在自然出现失效/验证时只读核对跨入口投影。
...
`tests/device/anonymous-readonly.ad` 使用开发版“临时匿名”屏蔽 credential 视图而不删除 Cookie：开始时先确认 NodeSeek 已登录；开启四站后，账号状态 Query 的旧登录结果不得覆盖搜索和写入消费者的匿名 view model，见 `REG-TEST-003`，并必须先看到 NodeSeek、linux.do 的未登录搜索提示；随后要求 NodeSeek、linux.do、小隐寺搜索有可打开结果，“全部”与三站首页有真实首条，妖火搜索和首页都进入 App 内登录限制；最后 relaunch 并确认 NodeSeek 登录恢复。它只能在身份匹配的当前开发包执行；正式 bundle 按 `MORE-05` 不显示测试工具，Release Smoke 明确排除该文件并执行其余七条。发布前两层都要通过，不得用未登录设备、清 Cookie、只看提示文案或把测试工具暴露进正式包替代。公开搜索受第三方反爬影响时，验收应记录为动态来源阻碍，不得把该轮结果写成固定零结果基线，也不得靠偷带登录 Cookie 或自动重试制造绿灯。
