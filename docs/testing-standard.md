...
`REG-TOPIC-094` 补充嵌套横滑验收：code/table 的真实 renderer RNTL 必须固定同一个 RNGH Pan 使用 `activeOffsetX([-10, 10])`、`failOffsetY([-10, 10])`、单指、被动 horizontal `Animated.ScrollView`、严格 clamp、`withDecay` 和无障碍增减滚动。一次 table segment Pan 后，所有同 scope + semanticId segments 与重挂实例必须得到同一 offset；纵向失败路径不得改变 x。匹配 APK 只读直达 linux.do `t/topic/2556285` 第 9 楼，分别验证慢速纯横拖稳定移动、代码区域纵拖滚动主题且 x 不漂、快速 fling 可用。
