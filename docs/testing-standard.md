...
`REG-TOPIC-096` 补充千图预览打开性能验收：2000 图 compiler 测试必须同时固定 `parseHtml` 恰一次和 `CompiledForumContent.previewImages` 完整保序；descriptor catalog 测试覆盖 lightbox、srcset/lazy aliases、尺寸、Referrer、inline 排除、去重及任意 index。Controller RNTL 不得提供 HTML，只能先注册 descriptors，再固定首次打开、关闭重开、等价 descriptors/空 inline map 不重建，内容、inline identity、宽度/DPR 与 media session/Referrer 变化才失效一次；Topic RNTL 必须证明主楼、回复 body/signature、已加载引用 body/signature 的目录不随筛选、排序、折叠或虚拟化改变。匹配最终 revision 的 Release APK 保留数据覆盖安装后，直达 NodeSeek `post-863650-1`，只点击首图并关闭重开三次；点击到完整 `1/1381` chrome `<=500ms`，disk-cached 当前图 `<=800ms`，不得临时显示 `1/1`、出现两秒空档、ANR/OOM/FATAL 或 PID 变化。该点击验收不遍历 25/100 页，不能替代 `REG-TOPIC-075` 的完整 PSS/Native Heap。
...
| 回复计数/稀疏窗口不一致导致整窗消失 | `tests/integration/source-read-contracts.test.ts`、`tests/ui/topic/topic-session-controller.test.tsx`、`src/sources/yaohuo/reader.test.ts` | 只读打开自然存在评论的各站 Topic；partial 行保持可见并可继续权威读取，不发回复或改原站状态 |
