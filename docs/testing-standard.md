...
| 登录 / 验证 / Cookie / Device Code / 凭据 | 每来源 `AccountSessionSnapshot` 是唯一账号事实；`identityTrust=confirmed|unknown|none`，`isVerifying` 只表示检查 activity。普通冷启动从 AsyncStorage 恢复 authenticated/anonymous 终态，Account probe=0；首次升级只对有 Cookie/SecureStore 候选的已启用来源核对一次并写全局 marker。store 仅允许 source/id/username/displayName/avatar/url，不含 Cookie/token/密码/Cookie 摘要/topics/活动且不进备份；单来源损坏隔离。检查开始/超时/网络/403/429/CF/解析失败保留原 snapshot/ReadPlan；A→A 只更新资料，明确 anonymous/A→B 才推进该站 epoch。登录 surface 打开只建立内存 barrier，零 probe/身份改写；关闭一次定向核对，终态后释放，失败本进程保持阻断；不持久化中断协议。首页 linux.do CF 是 read recovery，不进入 Account barrier，不改 identity/epoch/ReadPlan/query key，聚合保留 sibling，每次显式检测最多恢复 exact Query 一次。三站 Cookie 只由 Android `CookieManager` 按准确 URL 读取，响应不回写；只有用户显式“清除登录”可定向删除，本地账号退出绝不清 WebView Cookie。四站 verifier 仍只接受 current user、明确 anonymous 或 unknown；“内容可读”不是身份终态。小隐寺 Device Code/Key/Client ID 与重授权/撤销保持独立。秘密不进日志/备份 | `src/platform/storage/accountSessionStore.test.ts`、`src/domain/session/authSurfaceCoordinator.test.ts`、`src/platform/network/managedCookies.test.ts`、`tests/ui/account/account-status-controller.test.tsx`、`src/features/account/useVerificationController.test.ts`、`tests/ui/feed/feed-controller-xiaoyinsi.test.tsx`、`tests/ui/app/app-runtime-startup.test.tsx`、`tests/integration/query-session-contracts.test.ts`、各站 session/account tests、`tests/integration/security-boundaries.test.ts` |
...
```powershell
npm test -- tests/integration/source-read-contracts/ src/sources/xiaoyinsi/reader.test.ts src/sources/nodeseek/actionRequest.test.ts src/sources/nodeseek/actionClient.test.ts src/sources/xiaoyinsi/actionRequest.test.ts src/sources/xiaoyinsi/actionClient.test.ts src/domain/forum/topicActionState.test.ts tests/integration/hidden-browser-scripts.test.ts src/sources/yaohuo/reader.test.ts
...
```powershell
npm test -- src/sources/discourse/actionRequest.test.ts src/sources/discourse/model.test.ts src/sources/nodeseek/actionRequest.test.ts src/sources/yaohuo/actionRequest.test.ts tests/integration/source-read-contracts/ src/sources/yaohuo/parser.test.ts src/sources/xiaoyinsi/reader.test.ts
npm run typecheck
```
...
| 账号中心 | 已启用可登录来源按用户顺序、单站详情、真实状态、身份、主操作、顶部唯一公共 `刷新账号状态` 和全部原服务入口；刷新 activity 独立显示，检查失败保留上次确认身份，confirmed 才计登录、真正 unknown 计“待核对”；原三站显示凭据摘要并从 App 内打开登录 / 验证页，小隐寺只显示 Device Code 授权且打开系统浏览器；测试凭据填入但不提交并在结束前删除 | 清除网站登录、撤销授权、退出登录、手工改 Cookie、提交测试登录、真实签到 |
...
| 更多页 | 内容源五站开关、48dp 手柄长按拖拽、TalkBack 上移/下移、普通滚动/开关不被手势抢占；另检查版本、更新、账号中心、等级、服务器代理、诊断、备份和外观入口；内容源改动须记录并恢复原设置，诊断分享面板打开后取消 | 小隐寺等级复试、备份导出、导入、安装更新、切换外观、启用未知代理 |
