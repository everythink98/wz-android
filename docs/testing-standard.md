...
`REG-TOPIC-077` 补充详情/回复标准：`ReplyCompleteness=partial` 表示窗口不完整，不表示来源 adapter 已放入 `items` 的行无效。共享层只消费 `{ items, completeness }`，按 page 顺序连接 `items`，不得再用正文、作者、时间、旧 `replyCount` 或跨站 identity 规则过滤、合并或覆盖。adapter 已确认的 target window 不得被共享 predicate 二次否决；本地 matcher 只用于已加载集合查找和高亮。五站的集合成员、去重、floor、target 与 cursor 语义必须分别保留既有协议和兼容行为；是否继续读取只由明确 cursor 或用户动作决定，不能隐式创建后台任务。写后目标只能使用来源已确认的精确 `comment-id` 或完整 `delete-path`，floor 不承担删除身份兜底。V2EX 非空 partial 初次展示零 Reply transport，空 partial 一次主读取；显式刷新或未加载 target 各最多一次。complete 才允许权威总数、倒序和末尾确认；各 adapter 原有错主题、错页、错误 cursor、空窗口和 exact/write 边界继续严格。

`REG-TOPIC-062/063/067/068/069/070/071/072/073/076/077` 补充详情/回复标准：远端楼层定位采用锚点双向窗口，极大目标只请求目标窗口，用户触边才读取相邻 cursor；不得逐页追赶、扩大 page-size、补齐中间页或自动重试。共享层只投影 adapter 返回的 `items + completeness`，不新增跨站 validity、identity、membership、pagination 或 exact-target predicate。NodeSeek 保留 embedded comment collection、rendered row 和既有 location 投影语义，空 record 可展示，普通重复 comment ID 按原投影降为 partial，显式 floor target 不接受纯合成 floor。Discourse 普通 hydration 保留非空已请求 ID 子集并按 stream 顺序展示；缺失单条标 partial，重复或未请求 ID、整窗空缺和错误 cursor 仍失败，target 有 post ID 时按 ID，否则按 post number。妖火保留既有 reply selector 的每一行，正文、作者、时间为空不丢；`reid` 只用于原删除链接，导航与回复目标继续使用 floor，cursor 空页失败，权威 `replyCount=0` 的 start 可 complete empty。V2EX 按自身 DOM/API 完整性规则返回 complete 或 partial，partial 不自动读取。最低证据分别覆盖每站 adapter、Controller 不轮询、详情受影响提示和原有写/深链兼容行为；匹配 revision/APK 的设备验收只读执行。
