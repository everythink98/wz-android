...
| 功能 | 对的标准 | 常用自动测试 |
| --- | --- | --- |
| 入口 / 导航 | 冷启动进入首页；4 个底部入口可切换，每个按钮铺满现有底栏内容区内的等宽 slot、互不重叠且不改变图文与底栏几何，见 `REG-NAV-001`；从首页、搜索、收藏打开主题和用户页后可返回；详情页内再打开主题不会丢上一级状态。每个 native Topic route 的 session/list ref 独立，inactive route 非交互、不发 Query 且不复用旧 epoch 数据；图片预览、composer 和 native pop 的返回优先级固定 | `tests/integration/style-ownership.test.ts`、`src/features/topic/useTopicSessionController.test.ts`、`src/domain/forum/userNavigation.test.ts`、`tests/ui/app/app-navigator.test.tsx`、`tests/ui/topic/topic-session-controller.test.tsx` |
