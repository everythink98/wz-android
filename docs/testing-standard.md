...
`REG-FEED-014` 补充 Feed 关键路径证据：内部聚合读取复用调用方 diagnostic trace，每个 child 恰记录一次 `source + success/failure/timeout/canceled + latencyMs + sanitized reason`，不得记录 URL、Cookie 或用户数据。deferred Promise 必须固定最后一个 child 终态前结果绝不发布，保留 `Promise.allSettled`、每来源 5 秒预算和严格等待全部终态语义。匹配 Release APK 连续采集 5 次“全部”冷加载；同一来源至少 4 次成为最大值才报告为稳定瓶颈，transport 占比 `>=80%` 时不记作本地性能问题，最后来源终态到 Feed apply 中位间隙 `>300ms` 才进入下一轮本地合并/排序/提交优化。
