...
| 互动 / 写操作 | 未登录、identity pending、surface 开放或 ticket 过期时不发送；所有回复、编辑、删除、互动、投票、上传和签到统一经 `ensureWritableSession`，门禁在 Query snapshot、optimistic update、文件选择和 transport 之前，等待 Query cancellation、文件选择和写后刷新后再次校验，失败必须零 optimistic、零 upload、零 transport。原三站请求 Cookie 由原生只读 CookieJar 按准确 action URL 选择；JS 只构造 CSRF/sid 等协议字段，小隐寺只带独立 User API headers。只有站点 client/runtime 给出的 `login-required`、`login-expired`、`verification-required` 才请求一次 Account Query 对账；`ordinary` 与 `permission-denied` 只回滚本次 optimistic state、提示一次，零 barrier/epoch 变化。未确认失败即使令 ticket pending，也恢复仍存在的旧 scope snapshot；已经清除的旧 epoch Query 不得重建。任何失败都不自动重放非幂等操作，也不得删除原站 Cookie。服务器已确认后 ticket 失效时不再弹成功提示、应用迟到结果或写新 epoch cache，不回滚服务器已确认操作，诊断保留 `stale + serverConfirmed`。linux.do 与小隐寺的 Discourse 点赞/书签共享 MutationCache 与精确 Query cache：先局部显示，失败恢复原状态，确认后同步当前 route 的精确 Query cache；投票经服务器确认后局部更新且不整篇重载。NodeSeek 投票提交前确认，确认后严格 `POST × 1 → GET × 1`，GET 失败不重投、不伪造票数 | `src/domain/session/writableSessionGate.test.ts`、`src/sources/discourse/actionRequest.test.ts`、`src/sources/discourseActions.test.ts`、`src/sources/discourse/permissions.test.ts`、`tests/integration/source-read-contracts/`、`src/sources/xiaoyinsi/actionRequest.test.ts`、`src/sources/xiaoyinsi/actionClient.test.ts`、`src/sources/nodeseek/actionRequest.test.ts`、`src/sources/nodeseek/actionClient.test.ts`、`tests/ui/topic/topic-actions-controller.test.tsx`、`src/sources/linuxdo/actionClient.test.ts`、`src/sources/yaohuo/actionRequest.test.ts`、`src/sources/yaohuo/actionClient.test.ts`、`src/domain/forum/topicActionState.test.ts` |
...
| 登录 / 验证 / Cookie / Device Code / 凭据 | Account canonical Query 是远端身份唯一来源；workflow state 只承载登录 surface、验证/授权与本地 transaction。打开 NodeSeek/linux.do/妖火/NodeImage surface 只把该来源的 strict/private/write/notification 能力标为待核对，公开 ReadPlan 不受阻断，且绝不清、写或覆盖 Cookie；关闭按钮、系统返回、离开 More、切站及 NodeImage 结束都立即收起 UI 并异步对账，hidden/no-op、linux.do inactive 与权威结果自动关闭必须走负向矩阵，见 `REG-ACCOUNT-031`。三站 Cookie 只由 Android `CookieManager` 按准确 URL 读取；空 Cookie 与 error 分离，读取不调用 `flush()`，响应 `Set-Cookie` 不回写，App 不持久化或回退传输 header。只有用户显式“清除登录”可调用窄化 clear port 并回读确认，NodeSeek/linux.do 保留 CF。四站 verifier 严格三态：current user/self-account 为登录、契约明确匿名/准确游客结构为退出，其余均 unknown；pending 只对应真实 probe，普通失败或达到创建时确定、累计 25 秒 active time 的绝对终期后必须结算 terminal unknown；“内容可读”不是身份终态。NodeSeek 直连具证据时保持单请求，模糊 SSR 才进入 Account-only WebView 并等待 current user 或完整登录/注册游客控件；妖火公开 session 页 unknown 时补读精确登录 URL，只有完整 POST 登录 form 才是退出，缺字段仍 unknown；linux.do `_forum_session` 不得因缺 `_t` 被挡，NodeSeek 403/404 与妖火普通文字不得误判，见 `REG-ACCOUNT-037`。A→A 恢复该来源 strict 能力；A→B/A→anonymous/anonymous→B 原子推进目标站 epoch 并清理目标站及 `all` 私有 Query、Level/AI、Topic 服务端数据和媒体身份缓存；unknown 保留可信内容只读。Feed/Search/Topic/User 的公开 operation 在账号 pending/unknown 时继续以无 Cookie lane 结算，strict operation 只阻止自己且不能冻结 sibling；所有 queryFn 消费 `AbortSignal`。小隐寺 Device Code、Key/Client ID、重授权与撤销保持独立。秘密不进日志/备份，当前身份协议必须有官方或成熟实现证据 | `src/domain/session/authSurfaceCoordinator.test.ts`、`src/platform/network/managedCookies.test.ts`、`src/platform/storage/legacyCookieSnapshotMigration.test.ts`、`src/sources/nodeseek/session.test.ts`、`src/sources/linuxdo/session.test.ts`、`src/sources/yaohuo/session.test.ts`、`tests/ui/account/account-status-controller.test.tsx`、`tests/ui/account/account-controller.test.tsx`、`src/features/account/useVerificationController.test.ts`、`src/features/account/sessionQueryOwnership.test.ts`、`src/features/account/browserFetchQueue.test.ts`、`tests/integration/query-session-contracts.test.ts`、`src/platform/media/mediaSessionEpoch.test.ts`、`tests/integration/hidden-browser-scripts.test.ts`、`tests/integration/source-read-contracts/`、`src/sources/yaohuo/reader.test.ts`、`tests/ui/account/hidden-browser-host.test.tsx`、`tests/ui/feed/feed-controller-xiaoyinsi.test.tsx`、`tests/ui/search/search-controller-ai.test.tsx`、`tests/ui/topic/topic-session-controller.test.tsx`、`tests/ui/user/user-controller-session.test.tsx`、`tests/ui/account/account-controller.test.tsx`、`tests/ui/account/xiaoyinsi-auth-controller.test.tsx`、`src/sources/xiaoyinsi/auth.test.ts`、`src/platform/storage/credentialVault.test.ts`、`src/domain/session/loginFormAdapters.test.ts`、`tests/tooling/release-packaging.test.ts`、`tests/integration/security-boundaries.test.ts` |
...
`REG-DATA-007` 与 `REG-PROXY-011` 补充冷启动标准：App 只在统一 ReaderData 加载（其中包含有界的 `reader-settings` sidecar）尚未结算时显示静态、可访问且无动画的“阅坛 / 正在启动”，本地读取最多等待 3 秒；`reader-settings` 缺失、损坏、拒绝或超时后以五站全启用继续，并保留已成功读取的 ReaderData，迟到设置不再发布。routes 放行不等待代理；proxy SecureStore load 最多 3 秒，saved state 和 native apply 未结算、失败或 runtime 已结束时只有 managed network/WebView fail-closed，本地 Library/More 等 route 仍可使用。慢 native apply 不能被额外 JS deadline 误判为直连。
...
`REG-ACCOUNT-041/042` 补充账号标准：每来源稳定 `AccountSessionSnapshot` 是前台 JS 生命周期内唯一账号事实，Account key 不含内容 epoch，只有 `useAccountStatusController` 可提交；UI 与同步权限门禁必须读取同一 snapshot，不得保留 runtime/workflow 镜像。pending 只存在于真实在途 probe，任意错误和最终期限必须结算 terminal unknown 并释放页面。单 probe 期限从创建时起计算，复用者不得续期；批量 5 秒预算只停止本批等待。unknown 保留最后可信身份/资料但不计 confirmed 登录，strict/private/write/notification 保持关闭，公开 ReadPlan 继续。L 站成功 observation 在面板关闭前提交一次 confirmed；内容 epoch 随后推进也不得删除或搬运 Account snapshot，页面恢复失败只记录 error。用户显式重试才创建下一次 probe。
...
`REG-PERF-010` 补充海量正文验收：最低逻辑证据必须用一个 `<p>` 中 2000 张普通图片固定 `compileForumContent()` 的保序与每媒体 region 最多 4 张，并固定 poll/quote/video typed 顺序、普通/native-video parse-once、跨 typed segment 的统一逻辑节点 continuation 和 unsafe island fail-closed。FlashList region 是调度单元，不是任意内容的切割线：超旧字符/node 预算的无媒体连续富文本与 plain/terminal code 必须各保持一个语义 owner，table/list/details 等只在自然子项边界分段；模型与 renderer 不得保留旧 splitter、DOM quote extractor 或 video detector。主楼、回复/签名、评论完整引用、主楼内嵌完整引用和采纳答案展开正文都必须是父 FlashList direct regions；在 renderer 内 mock lazy 或只断言 Dispatcher 上限均不足。viewability RNTL 必须固定未完成工作 warm `<=8`、running `<=4`、正文原图 `<=1`、未获 permit 零 source/Call、全 route 单 deadline Timer、每个失败 identity 只有一次受门禁约束的自动重试、第二次失败后零滚动/recycling/runtime 第三波及一次显式重试；runtime generation 只能由 coordinator 重启当前 running attempt，displayed/waiting/failed 不变。图片成功显示后连续离窗/回窗不得退回 idle 或增加请求波，视觉/recycling key 必须只由稳定媒体 identity 决定；attempt 只切换网络 source 并拒绝迟到事件。贴纸/link-card/iframe/video 保留各自既有 attempt 生命周期。正文 SVG consumer 必须随未完成 attempt、region 与 route lease 一起释放；最后一个 consumer 释放后 queued artifact work 不启动、可取消 fetch abort、不可取消 Native 读取返回后不进入 poster，而同 identity 的其他活跃 consumer 仍共享一次 fetch/poster。全局 bounded artifact service 不得错误归属到 coordinator。聚合测试必须固定 planned/media 与 warm/running/timer session 高水位、response-ready 到首个 opening region layout 的 `firstRowElapsedMs`、Topic A→B 隔离和 exactly-one finish。预览用 2000 项 catalog 固定 physical ring slot `<=3`、所有 current/adjacent raster 与 poster 均为 `cachePolicy=disk` 且 `allowDownscaling=true`、只更新离屏槽 source、stale logical event 不改变新 item、inactive 异常页激活后恰重试一次、无进展失败卸载 load layer，且首、中、末 index 可达。匹配 x86_64 Release APK 必须按“重新打开 App → 收藏 → 历史 → 整活帖”进入，返回历史后再点同一项完成 warm-cache 第二轮，再打开普通详情；不得用 deep link、清数据或改网络代替。记录历史点击到导航 commit、首 region/Back、running/warm/queued/timer 高水位、gfxinfo/帧、PSS、attached View、ANR/OOM/FATAL/PID 和 ResponseBody leak。只有同时满足 `REG-PERF-010` 记录的时间、帧、内存和资源门槛才记 `LIVE_PASS`；公网不可用只记 `NOT_VERIFIED`。

`REG-PERF-013/014/015/016/017` 补充全项目稳定输入验收：1413/2000 图 fixture 必须同时固定正文 parse=1、compile=1、单次 post-order semantic analysis、零 subtree reanalysis、最终 sanitized root 只序列化一次、普通巨图段落零 sticker HTML 序列化、每 row 最多 4 个网络媒体和完整 preview 顺序，并用受控 `URL` constructor 证明每个唯一非空候选最多解析一次；lightbox、srcset、lazy、data URL、placeholder、invalid URL、emoji、sticker、Referrer 和动态 inline 输出继续等价。普通图片继续走唯一 richText 图片 renderer；不得为了纯图另建 typed renderer。NodeSeek Feed/direct Search/Google/Topic/当前账号、妖火列表/回复/User、LinuxDo Google 和 V2EX 详情每个正常响应只 parse 一次；每条最终 source content 的 sanitize/quote/compile 共用一个 root，畸形 fallback 单独计数。LinuxDo Feed/Search 在首个服务端页面已有 `limit` 条有效结果时只读该页并信任 cursor；同 scope 的 Catalog、Feed、Search 合计最多一次 `/site.json`，scope 变化恰好一次新请求且不得泄漏旧身份分类。App runtime RNTL 必须固定 ReaderData 后 Feed/Categories 立即启动、普通 Feed 只在 `onLoad` 与 Categories 终态后启动初次账号/更新、账号逐站结算期间 Feed/Categories gateway 与 session epochs 身份不变、最终最多切换一次，通知远端刷新再等待账号批次。同一 QueryClient 中卸载并重挂相同 Topic 必须复用 RAM 数据，detail transport 和正文 compile 不增加；外部 link 测试只覆盖 navigation-ready 排队正确性。Reader index 必须由 Feed/Search/Library/User 共用，favorites/history/listDensity 每 revision 只重建一次，无关变化零重建；Bottom Tabs 初始只挂当前页、首访一次、返回保留状态且 inactive freeze，`REG-ACCOUNT-043` 的真实 blur 清理继续单独覆盖。匹配 Release APK 执行 5 次 process-cold 普通启动，并从真实 Feed/Library 路径首次进入、返回、重入 Topic；按 request-start diagnostics、body-ready、source-parsed、content-plan-ready、catalog ready、first row/layout、first media request 和 compiler 聚合记录验收。没有匹配基线或公网不稳定时只能记 `NOT_VERIFIED`，不得用单次主观体感宣称门槛通过。
...
`REG-ACCOUNT-031` 补充 surface 与 epoch 验收：NodeSeek、linux.do、妖火、NodeImage 的 open/close-button/hardware-back/navigation-away/switch-surface/success/cancel/authoritative-recovery 必须覆盖完整矩阵；open 只把该来源 strict/private/write/notification 标为待核对，公开 ReadPlan 不受阻断，且零 clear/flush/身份覆盖，hidden close 与 linux.do inactive 是 no-op。登录/注册 URL 本身不能证明退出，错误页或半加载页必须保持 unknown。关闭后的 A→A/A→B/A→anonymous/unknown 必须通过临时 probe key 原子提交；迟到 generation 不得落地。A→B/anonymous 推进目标站 epoch 并清理 source/`all` 私有 Query、Level/AI、Topic 服务端 data 和 media identity，A→A 不清 cache，unknown 保留旧 data 只读。consumer RNTL 分别固定 Feed/Search 的公开来源（包括同一 pending 来源）继续结算；当前 public scope 的可信条目可保留，但 authenticated 条目不得跨 scope 投影；epoch 变化后立即停止复用；Topic/User/Level/AI 暂停新私有请求；User route 只保留定位字段；媒体 cacheKey/player 随 epoch 重建。
...
`REG-TOPIC-084` 补充表格与虚拟化验收：compiler 测试必须用 V2EX 式 18 行两列表证明可满足硬预算的完整 table 形成一个 typed segment，并用更大 fixture 固定只在完整 `tr`/rowspan 连通区域之间分段、统一 columns、唯一 thead、行序完整及来源伪造属性无效。typed document/Kotlin 测试必须固定 NodeSeek 式 3/2 列铺满正文、独立 table 间 `12dp`、`rowspan/colspan`、字号缩放、主题样式、6 列横滑、同一 `scope + semanticId` 分段零接缝与 offset 同步/重挂恢复；既有 scope 测试继续覆盖主楼、回复、引用、签名和采纳答案隔离。不得用关闭 FlashList、合并巨型 cell、扩大预加载或 WebView 代替。
...
`REG-TOPIC-094` 补充嵌套横滑验收：code 继续由现有 RNGH manual Pan + 被动 `Animated.ScrollView` 实现；Compose typed table 由选择面内的原生 pointer owner + 被动 `horizontalScroll` 实现。两者必须保持同一行为契约：单指、`4dp` 方向锁、达到阈值后仅 `|dx| > |dy|` 接管，纵向占优或相等、多指及无 overflow 让行，offset 严格 clamp、松手 fling、无障碍增减滚动；一次 table segment 横拖后所有同 `scope + semanticId` segments 共享 live offset，全部回收后重挂仍恢复。RNTL 固定 code，Kotlin 固定 table 仲裁与 typed scroll key，匹配 APK 只读直达 linux.do `t/topic/2556285` 第 9 楼及 V2EX `t/1233470`，分别验证慢速纯横拖、纵拖主题且 x 不漂、快速 fling 与分段同步。

`REG-TOPIC-097` 补充 Android selectable 内容手势所有权验收：RNTL 从真实 code renderer 驱动 RNGH StateManager，Kotlin 从真实 table decision 固定 `<4dp` 未决、横向占优只接管一次、纵向/相等/多指/途中加指/无 overflow 让行，并继续固定 selectable、完整复制、查询高亮、terminal Tab 与无障碍动作。监督式 Agent Live 以录屏和 UI hierarchy 固定 NodeSeek `post-812712-1` 的 `240px / 5s` 代码慢拖零放大镜、handles 和选择菜单，静止长按仍能选择；V2EX table 同样要求横拖不触发选择、静止长按仍可选单元格，外层纵向滚动和四个 terminal Tab 不降级。坐标手势不包装成 tracked Replay；平台 recognizer 不能由 Jest/Kotlin 单测代替，因此没有匹配 APK 时不得宣称完整通过。
...
4. Gateway/RNTL/导航测试固定 More 入口和 `none/update/messages/both` 无障碍文案、总览/单站/未读筛选、合法 outcome marker、分页/刷新/局部错误、详情、已读失败提示、首次 opt-in、权限拒绝/撤销意图及四站开关；聚合局部错误必须为每个失败来源显示独立重试，并证明点击只调用目标来源 `listPage`、携带发起时 expected identity 与 cancel signal、只 patch 对应失败页；失败页不是末页时恢复 cursor 必须传播到末页，其他来源数据和 `listAllPage` 不变。还必须覆盖消息 native route 的 Android 硬件返回、旧 `read,write` 小隐寺授权显示升级而非登录且不能维持空后台任务、其 scope 读取不阻塞其他来源身份、pending/unknown 保留可信身份/cache 但不参与读取也不渲染旧私有 row、确认退出清 aggregate Query、前台 snapshot 调用共用投递 worker、snapshot 存储失败不阻断成功来源、消息中心可见时仍调用 Android system sink、隐藏但 mounted 的列表停止读取、快速连续身份变化只允许最新 effect 落盘、详情 route 绑定条目身份，以及 adapter I/O 前的 expected identity/abort；主题级 target 打开 Topic 时不得携带 `targetReply`，精确帖子 target 必须保留完整定位字段；条目 label 包含动作，短来源 Tab 双轴至少 48dp。
   ...
   - `REG-NOTIFY-058` 固定每个 `source + identityKey` 独立 snapshot Query：A 扩为 A+B 时 A 总读取一次、B 一次；同一 `dataUpdatedAt` 只持久化和触发对应 foreground delivery 一次。初次账号批次完成前零远端 snapshot；显式刷新、App 恢复和定时刷新仍对当前每来源各增加一次。
...
| 账号中心 | 已启用可登录来源按用户顺序、单站详情、真实状态、身份、主操作、顶部唯一公共 `刷新账号状态` 和全部原服务入口；pending 必须有真实 probe，失败/最终期限后结算可重试 unknown，confirmed 才计登录、unknown 计“待核对”；原三站显示凭据摘要并从 App 内打开登录 / 验证页，小隐寺只显示 Device Code 授权且打开系统浏览器；测试凭据填入但不提交并在结束前删除 | 清除网站登录、撤销授权、退出登录、手工改 Cookie、提交测试登录、真实签到 |
...
| 账号 pending/unknown 锁死公开读取或串用 authenticated cache | `src/domain/forum/readPlan.test.ts`、`src/sources/readGatewayContract.test.ts`、`tests/integration/query-session-contracts.test.ts`、Feed/Search/Topic/User controller UI tests | 自然账号核对期间检查公开 Feed/Search/Topic/User 不显示永久暂停；妖火 strict 保持可恢复登录提示 |
...
| Account probe 永久 pending、重复 owner、L 验证后不同步或 unknown 计为登录 | `src/domain/session/siteSessionState.test.ts`、`src/features/account/sessionQueryOwnership.test.ts`、`tests/ui/account/account-status-controller.test.tsx`、`tests/ui/account/account-center.test.tsx` | 账号中心刷新一次，逐站形成 confirmed/none/unknown 终态；L 普通检查后无需公共刷新即同步；不为制造 timeout/CF 断网或清 Cookie |
