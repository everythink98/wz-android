...
| 登录 / 验证 / Cookie / Device Code / 凭据 | `SiteSessionState` 是凭据 transaction、登录弹层和验证/授权 workflow 的本地状态；账号中心四站远端 data/error/busy 直接由独立 Account Query 派生，刷新结果不得再写入第二份 session projection。小隐寺 Account Query 必须使用不发布 workflow event 的只读授权结果，见 `REG-ACCOUNT-016`。公共刷新和启动恢复中单站凭据或身份读取失败不得阻断其他站，失败 Query 保留上次可信 data 并只给该站显示 error；`cookie-loaded` 只观察本地凭据或身份，验证恢复后的 `verification-succeeded` 也只观察最终状态，两者都不得重置 Query。实际替换凭据或 Device Code 授权完成发送 `session-updated`，后者与登录确认变化、失效和清除只重置对应 source 与 `all` Query，并由新的 Query result 同步该来源 data/loading/error，其他来源不变；linux.do 清旧 clearance 的 `session-updated/cleared` 与保存新凭据的 `session-updated`，只有在对应结构化 Query key 仍 exact active 且该 recovery 未被替换时，才携带完全相同的 `recoveryQueryKey`。QueryClient 必须对 `recoveryQueryKey` 做 exact 匹配：首屏观察新 credential key，分页、回复或引用保留已加载 pages/cursor 后再合并；不再 active、前缀相似或其他 lane 一律正常失效。明确失效不得被通用 `cleared` 覆盖。凭据摘要读取允许 partial，成功站摘要必须应用、失败站保留旧值。原三站 Cookie 与保存凭据互不删除，账号密码仅进 SecureStore且只在可信字段填入；WebView bridge 消息必须校验对应站点 HTTPS origin。NodeSeek/妖火清登录是 SecureStore 与 WebView Cookie 的 multi-store 提交，全部成功后才能发布 `cleared`，部分失败发布 `check-failed` 且保留原 typed 错误；NodeSeek/linux.do/妖火动作确认失效后的自动清理失败也必须保持 `login-expired` 并提示重试。linux.do clearance 只是恢复候选，原读取普通失败时 verification overlay 保持可重试且不报告成功。小隐寺 App 会话只由 User API Key、Client ID 和 `/session/current.json` 决定，系统浏览器仅承载一次性 Device Code 授权页，Cookie 不进入其状态判断；被动授权复核发送观察事件，新授权完成才发送 transition。Token 已保存后若首次 session 普通复核失败，重试必须先恢复现有授权，不能重复生成 Device Code；重授权终止后的旧会话普通复核失败保留 `check-failed`，只有 401/403 明确失效后才清理或重新授权；写操作授权复核失败不得覆盖原始操作错误。RSA 私钥不导出 Keystore，User API Key、验证码、nonce 和授权 URL 查询参数不进日志或备份；页面提示区分未登录、授权中、失效、拒绝、过期和普通失败 | `tests/ui/account-status-controller.test.tsx`、`tests/ui/account-controller.test.tsx`、`src/app/useAccountCredentialController.test.ts`、`src/app/accountCredentialDiagnostics.test.ts`、`src/app/useVerificationController.test.ts`、`tests/ui/topic-actions-controller.test.tsx`、`src/siteSessionState.test.ts`、`src/app/sessionControllerHelpers.test.ts`、`src/app/serverState.test.ts`、`src/sources/sourceGatewayContract.test.ts`、`src/xiaoyinsiAuth.test.ts`、`tests/ui/feed-controller-xiaoyinsi.test.tsx`、`tests/ui/search-controller-ai.test.tsx`、`tests/ui/topic-session-controller.test.tsx`、`tests/ui/user-controller-session.test.tsx`、`tests/ui/xiaoyinsi-auth-controller.test.tsx`、`src/credentialVault.test.ts`、`src/loginFormAdapters.test.ts`、`src/screens/more/accountCenter.test.ts`、`src/nodeseekCookies.test.ts`、`src/yaohuoCookies.test.ts`、`src/cookieCleanup.test.ts`、`src/appSecurity.test.ts` |
