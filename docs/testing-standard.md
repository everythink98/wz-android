...
| 首页 / 分类 / 分页 | 五站来源按当前支持范围返回；分类不串站；分页不重复、不漏掉下一页，已缓存页仍须追加到当前列表；同 key 并发读取只发一个 transport 且由最新调用方应用；聚合首页保留来源平衡，单站请求或凭据存储失败只形成该来源错误；单站刷新错误或带 `parse_empty` 诊断的首屏/分页不得应用，失败页保留旧列表和 cursor，聚合分页不混入半页；真实会话变化清该站和聚合投影、释放 Loading 并主动重载当前受影响来源，其他站不变；linux.do、NodeSeek、V2EX、小隐寺单站排序参数和缓存 key 不串用；小隐寺 latest/hot/new-all/new-topics/new-replies 分别命中真实路径与 subset，非空列表切换也必须重新请求 | `src/app/useFeedController.test.ts`、`src/app/serverState.test.ts`、`src/feedLogic.test.ts`、`src/feedCategoryRail.test.ts`、`src/forumApi.test.ts`、`src/sources/sourceGatewayContract.test.ts`、`src/localSources.test.ts`、`src/localXiaoyinsi.test.ts`、`tests/ui/feed-controller-xiaoyinsi.test.tsx` |
| 搜索 | 空关键词或请求进行中不重复提交；同 key 并发读取只发一个 transport 且由最新调用方应用；“全部”按 V2EX、linux.do、NodeSeek、妖火、小隐寺固定顺序显示每站最多 2 条预览且不分页；单站直接显示连续完整列表并可分页，已缓存下一页仍须展示；结果字段完整；错误按站点隔离；带 `parse_empty` 诊断的页面不得覆盖结果或推进 cursor，只重试原失败页；真实会话变化只移除对应来源普通/AI 投影并释放 Loading；筛选参数真实传给站点；小隐寺标准 Discourse 筛选必须使用本站分类/标签/作者候选和独立 User API 凭据，包含原站解决/未解决状态，不出现 linux.do 专属专家回应或 AI；登录态限制必须显示站点提示；NodeSeek 登录时走站内搜索，未登录时允许受限 Google 搜索结果，且两种状态要分开记录；小隐寺匿名搜索不被授权状态阻断 | `src/app/useSearchController.test.ts`、`src/app/serverState.test.ts`、`src/forumApi.test.ts`、`src/localSources.test.ts`、`src/localXiaoyinsi.test.ts`、`src/searchFilters.test.ts`、`src/searchListItems.test.ts`、`src/sources/sourceGateway.test.ts`、`src/sources/sourceGatewayContract.test.ts`、`tests/ui/search-screen.test.tsx`、`tests/ui/search-controller-ai.test.tsx`、`src/yaohuoApi.test.ts` |
| 详情 / 回复 | 标题、正文、作者、时间、分类、回复数和权限提示正确；同 key 并发读取只发一个 transport 且由最新调用方应用；带 `parse_empty` 诊断的详情、回复页或引用帖不得应用；回复分页不丢楼层或推进失败 cursor，已缓存下一页仍须展示；V2EX 评论刷新委托整篇读取时，只有 Topic Query 的 `dataUpdateCount` 推进才记录成功；真实会话变化清除对应来源旧详情与 route snapshot、释放 Loading 并保留可重试目标；正文引用和评论引用分别默认显示简介，展开后显示目标完整帖子，且两条渲染路径互不串样式、状态或缓存；同一引用的新请求必须使旧响应、旧 Loading 收尾和旧验证恢复失效，但同一 linux.do 验证链可精确重试；块级正文图片冷加载只有一个全宽 4:3 占位和一个连续 Spinner，同一 ImageRef 就绪后直接显示，热重进保持真实比例，请求切换不泄漏旧图，inline 媒体不进入块图 loader；图片预览可用且快速重复保存只写入一次；返回后上一层详情状态保留 | `src/app/useTopicController.test.ts`、`src/app/serverState.test.ts`、`src/quotedPosts.test.ts`、`src/topicSessionState.test.ts`、`src/topicDerivedData.test.ts`、`src/topicContentSplit.test.ts`、`src/topicContentHtml.test.ts`、`src/topicListItemState.test.ts`、`src/localSources.test.ts`、`tests/ui/topic-session-controller.test.tsx`、`tests/ui/topic-image-loading.test.tsx`、`tests/ui/image-preview-controller.test.tsx` |
...
| 登录 / 验证 / Cookie / Device Code / 凭据 | `SiteSessionState` 是四个可登录来源的唯一 UI 状态投影；公共刷新和启动恢复中单站凭据读取失败不得阻断其他站，身份读取失败发送 `check-failed` 并保留上次可信身份，不得随后用 `cookie-loaded` 覆盖；`cookie-loaded` 只观察本地凭据或身份，验证恢复后的 `verification-succeeded` 也只观察最终状态，两者都不得重置 Query。实际替换凭据或 Device Code 授权完成发送 `session-updated`，后者与登录确认变化、失效和清除只重置对应 source + aggregate Query，并同步该来源页面投影和 Loading，其他来源不变；linux.do 清旧 clearance 的 `session-updated/cleared` 与保存新凭据的 `session-updated` 只有在同一 recovery 仍 current 时才携带完全相同的 `recoveryKey`。controller 必须同时精确匹配 key、source 和 lane：首屏清投影后重读，分页、回复或引用保留已加载数据和 cursor 后再合并；stale、前缀相似或其他 lane 一律正常失效。明确失效不得被通用 `cleared` 覆盖。凭据摘要读取允许 partial，成功站摘要必须应用、失败站保留旧值。原三站 Cookie 与保存凭据互不删除，账号密码仅进 SecureStore且只在可信字段填入；WebView bridge 消息必须校验对应站点 HTTPS origin。NodeSeek/妖火清登录是 SecureStore 与 WebView Cookie 的 multi-store 提交，全部成功后才能发布 `cleared`，部分失败发布 `check-failed` 且保留原 typed 错误；NodeSeek/linux.do/妖火动作确认失效后的自动清理失败也必须保持 `login-expired` 并提示重试。linux.do clearance 只是恢复候选，原读取普通失败时 verification overlay 保持可重试且不报告成功。小隐寺 App 会话只由 User API Key、Client ID 和 `/session/current.json` 决定，系统浏览器仅承载一次性 Device Code 授权页，Cookie 不进入其状态判断；被动授权复核发送观察事件，新授权完成才发送 transition。Token 已保存后若首次 session 普通复核失败，重试必须先恢复现有授权，不能重复生成 Device Code；重授权终止后的旧会话普通复核失败保留 `check-failed`，只有 401/403 明确失效后才清理或重新授权；写操作授权复核失败不得覆盖原始操作错误。RSA 私钥不导出 Keystore，User API Key、验证码、nonce 和授权 URL 查询参数不进日志或备份；页面提示区分未登录、授权中、失效、拒绝、过期和普通失败 | `src/app/useAccountStatusController.test.ts`、`src/app/useAccountController.test.ts`、`src/app/useAccountCredentialController.test.ts`、`src/app/accountCredentialDiagnostics.test.ts`、`src/app/useVerificationController.test.ts`、`src/app/useTopicActionsController.test.ts`、`src/siteSessionState.test.ts`、`src/app/sessionControllerHelpers.test.ts`、`src/app/serverState.test.ts`、`src/sources/sourceGatewayContract.test.ts`、`src/xiaoyinsiAuth.test.ts`、`tests/ui/feed-controller-xiaoyinsi.test.tsx`、`tests/ui/search-controller-ai.test.tsx`、`tests/ui/topic-session-controller.test.tsx`、`tests/ui/user-controller-session.test.tsx`、`tests/ui/xiaoyinsi-auth-controller.test.tsx`、`src/credentialVault.test.ts`、`src/loginFormAdapters.test.ts`、`src/screens/more/accountCenter.test.ts`、`src/nodeseekCookies.test.ts`、`src/yaohuoCookies.test.ts`、`src/cookieCleanup.test.ts`、`src/appSecurity.test.ts` |
