...
| 详情 | 来源、标题、作者、时间、正文、回复数、附件、图片预览、返回；含图长帖按 `REG-TOPIC-048` 检查适屏先显、附近渐进清晰、远段不抢加载、全屏返回外层同步清晰且不跳位、原图失败保留适屏图；正文引用与评论引用分别检查简介常显、展开完整帖、收起恢复简介，并确认修改一处后另一处样式和交互不变 | 保存图片、回复、点赞、收藏切换 |
