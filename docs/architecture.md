...
- Feed 和 Categories 进入页面立即执行一次 direct request，冷启动 Account 刷新只作为 background 并发执行，不增加登录预检。只有可信 Cloudflare challenge 或既有明确网络兜底条件才进入一次隐藏 WebView fallback。NodeSeek 与 linux.do 各自使用稳定队列，优先级为获授权写操作、前台可见读取、后台账号刷新，同优先级 FIFO；已开始任务不被后来任务抢占，单个 Abort 只取消自己的任务。排队时间不消耗隐藏 WebView 的执行超时，获得执行权时才开始计时。见 `REG-SOURCE-005/006`。
- 读取已保存凭据和原站身份只发布 `cookie-loaded` 观察事件，不得取消当前 Query；小隐寺的被动授权复核也遵守这一语义。只读取 Cookie/SecureStore 的被动事件必须省略 `loggedIn`，只能刷新凭据摘要，不能覆盖已确认登录、失效或正在进行的验证/授权；当前账号协议明确给出身份结论时才携带 `loggedIn: true/false`。新凭据提交、Device Code 授权完成发布 `session-updated`，登录确认变化、凭据过期和明确清除也是会话 transition。`useSessionController` 对这些 transition 通过 QueryClient 精确取消并移除对应 source 以及可能混入旧私有数据的 `all` 聚合 Query；其他来源保持不变，聚合 Search 中未受影响来源继续结算。见 `REG-ACCOUNT-023`。
- linux.do 打开验证时只记录当前候选基线，不清理原站 Cookie；只有 recovery 引用未被替换且对应结构化 Query key 仍有 active observer，用户检测保存新候选时的 transition 才携带该 `recoveryQueryKey`。QueryClient 只保留与这个 key `exact` 匹配的一条 Feed/Search/Topic/User/Level Query，前缀相似、其他 lane、其他来源和已失去 observer 的 key 都照常清除；分页、回复和引用因此保留已加载 pages/cursor 作为恢复基线。登录/验证 WebView 的 load、probe 和 message 只允许更新当前面板的候选值、readiness 与诊断，不得保存凭据、发布 canonical session transition、调用 recovery 或关闭面板；每次已结算的用户检测都重新读取当前 Cookie/凭据，且同一时刻只执行一次。linux.do 只有同一 exact recovery 的原读取成功才自动关闭，仍为 CF 或普通失败则保留面板供下一次显式检测；Account 手动入口没有 recovery，检测成功后保持打开。验证控制器不拥有或无条件清理 Level cache；恢复成功后的 `verification-succeeded` 只是状态观察，不得再次失效 Query 或擦除刚恢复的数据。小隐寺 Device Code 轮询与 NodeImage OAuth 回调是协议终态，不属于 WebView 检测按钮。见 `REG-LINUXDO-002`、`REG-LINUXDO-006`、`REG-VERIFICATION-001`、`REG-TOPIC-022`。
...
- `src/app/useAccountController.ts` 负责 NodeSeek、linux.do、妖火登录 / 验证页检测、App 内凭据候选保存、用户明确触发的登录清理和 linux.do 等级读取；候选只服务于身份协议、generation、CSRF、sid、touserid 等状态，不负责给原生请求传输 Cookie。NodeSeek 与 linux.do 每次手动 probe 只接受当前 WebView 文档的私有 nonce 与 documentKey 三态结果，任何新文档 `onLoadStart` 都先作废旧 probe。
- 登录 WebView 的组件身份只由面板实例和显式 renderer 恢复 generation 决定；凭据填入 attempt 只用于关联 probe / fill 回执，必须通过已挂载 WebView 的 `injectJavaScript` 发送，不能进入 React key 销毁当前页面。NodeSeek 专项清理按 Cookie 的 host/domain/path 身份同时过期 host-only 与 `Domain=nodeseek.com; Path=/` 登录 Cookie，`flush` 后回读确认目标名称均已消失；`cf_clearance` 和其他站 Cookie 不在该事务内。见 `REG-ACCOUNT-022`。
- `src/app/useSessionController.ts` 只负责加载 Cookie 和会话事实；NodeSeek Cookie 加载只返回本次凭据里的 userId，不顺带读取个人资料。普通 Feed/Search/Topic/隐藏 WebView 触发的凭据加载是被动观察，不拥有身份判定权，也不能清除已确认的 current user。
...
- NodeSeek 当前账号只由当前会话页面证明：优先只读取服务端注入的 `__config__.user`，不递归接受同一配置内的 `profile/detail/member` 等公开或无关对象；再读取当前首页或设置页的专属 self-account 结构。不存在的 `/api/account/getInfo?readme=1` 不得被当作当前账号端点；保存的 userId 和 `/api/account/getInfo/{id}` 公开资料只用于用户页与显示，不能证明登录。只有当前页面准确呈现登录、注册游客控件时才提交 `login-expired`；HTML 404、普通网络、CF、超时或解析不确定都只能产生 `check-failed`，不得授权清除登录 Cookie。见 `REG-ACCOUNT-024/025`。
- 四站不共享验证器，但共享登录证据语义：`logged-in` 必须由当前凭据端点的 current user 或站点专属 self-account 结构证明，`logged-out` 必须由该站协议明确约定的匿名结果证明，其余均为 `unknown`。Cookie、旧 userId、Topic 作者、公开资料和普通业务文案不能证明身份；Cookie 名也不能在请求前替代或阻止当前会话验证，只要存在本站可用候选就交给该站 verifier。NodeSeek 使用当前页面的 `__config__.user` / self-account 结构与真实 `/signIn.html`、`/register.html` 游客控件；linux.do 的 Cookie 会话以 Discourse `/session/current.json` 的 current user 为登录证据、官方匿名 `404` 为退出证据，`401/403` 不授权清理；小隐寺在 User API Key 下以同一端点的 current user 为登录证据，只把 Discourse JSON `403` 且 `error_type=invalid_access` 或显式匿名字段视为退出证据，原始 `401/403/404` 及其他 JSON 错误保持 unknown；妖火以成熟客户端同样读取的 `wapindex.aspx?sid=-2` 中 `div.top2` 本人导航及 `touserid` 为登录证据，HTTP 状态、公开卡片和普通“我的/欢迎”文案均不证明退出或登录。妖火手动检测允许 `sidyaohuo`、`ASP.NET_SessionId`、`GUID` 中任一已支持 Cookie 组合进入真实验证，不能在请求前用某个 Cookie 名判登录；即使 adapter 名义上返回成功，没有 current user 也必须保持 unknown。妖火资料页只补全显示信息，站点昵称可替换初次验证得到的数字 ID 占位，补全失败不得推翻已经证明的最小当前身份。App 保存的 Cookie 摘要只作为 verifier 候选和协议数据；读取、资料补全和写请求均由原生只读 CookieJar 按准确 URL 从当前 WebView Cookie store 取值，不再发送持久化 Cookie header。`unknown` 不清凭据、不发布登录/退出，并保留 Account Query 的上次可信 data。见 `REG-ACCOUNT-025/026/029`。
- 原站 WebView 的 Cookie jar 由网站与 Android `CookieManager` 持有，是 NodeSeek、linux.do、妖火请求 Cookie 的唯一事实来源。React Native 请求统一使用 `credentials: include`，原生 `ReadOnlyWebViewCookieHandler` 只对三个受管域调用 `CookieManager.getCookie(准确 URL)`，交由平台处理 Domain、Path、Secure 与重定向选择；响应 `Set-Cookie` 的保存入口为 no-op，不能回写 WebView。调用方不得维护 Cookie 名白名单、拼接 header、缓存传输值或在实时读取为空时回退旧快照。RN Networking、Fresco 与 Expo Image 使用同一个 managed OkHttp client；Expo Video 在创建 source 前通过原生桥读取一次准确媒体 URL 的实时 header，且不持久化，读取异常时不创建匿名 source。账号检测、刷新、过期判定、fallback 和普通失败不得写入或删除原站 Cookie；只有用户明确点击该站“清除登录”时才允许定向过期并回读确认。Android 同进程的可见与隐藏 WebView 共用 `CookieManager`，NodeSeek/linux.do 隐藏 fallback 首跳只给 URL；原生桥只调用公开 `CookieManager` API，不读取 WebView 私有数据库。见 `REG-ACCOUNT-026/027/029`、`REG-TOPIC-019`。
...
- 保存、清除、换号或重新授权等外部会话变化会 reset 目标站 active/disabled Account Query、清目标站及 `all` 私有缓存并推进该站 credential scope；其他三站不变。若 Account Query 自己确认失效，必须先在旧 exact key 提交 `expired`，再只把这一份失效结果 seed 到新 scope key；不得用全局 previous data 把旧 `logged-in` 带到普通新 scope。小隐寺 Account Query 的这一只读收口不发布 Device Code workflow event。
...
- Android 原生代理模块由 `plugins/withNetworkProxyModule.js` 写入生成目录，并通过 `app.json` 的 plugin 列表持久化。它在 `OkHttpClientProvider` 的同一 client builder 上同时安装代理 `ProxySelector` 与只读 WebView CookieJar；该 Jar 满足 RN `CookieJarContainer` 类型契约，但拒绝 Networking/Fresco 生命周期把 delegate 换回可写 handler。Networking、Expo Image 和显式 managed client 复用同一 selector、dispatcher、connection pool 和 CookieJar，不创建绕开代理的平行 client。见 `REG-ACCOUNT-030`。
