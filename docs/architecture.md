...
- 详情 session 由 `useTopicSessionController` 聚合，并通过 `src/features/topic/model/sessionState.ts` 快照保存和恢复；互动 controller 使用回复成功收尾、草稿图片插入和 action 状态更新等领域命令，不直接修改详情 session 的 state。嵌套详情、Topic→User→Topic、Android 物理返回、草稿和滚动位置是必须保留的行为。
