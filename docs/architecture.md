...
- NodeImage 自动授权先对 NodeSeek 做 canonical Account preflight 并捕获 `{identityKey, sessionEpoch}`；只在精确 NodeSeek Connect 顶层文档注入无 secret 的 cAuth probe，只在精确 NodeImage 顶层文档加载完成后注入局部 IIFE payload。每次 flow 优先用 Web Crypto `getRandomValues` 生成 128-bit nonce；当前 Hermes 未提供 Web Crypto 时复用现有原生 `SecureRandom` port，两个安全来源都不可用或结果无效即失败关闭，绝不降级 `Math.random`，所有消息必须回显 nonce。异步 nonce 初始化 single-flight；切换 auth surface或关闭会取消 pending opening。最终 reconcile 仍为同一 owner 且 epoch 未变才可按原 credential generation 保存 Key；SecureStore 写入前后再次核对 owner/epoch，期间换代恢复写入前凭据。changed、anonymous、unknown、stale 或迟到 generation 均零写入，见 `REG-ACCOUNT-010`。
