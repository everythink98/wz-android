...
- `src/app/AppNavigator.tsx` 是唯一的路由拓扑事实源且不依赖 feature；`AppRoutes` 把六个 route entry 注入其中。`useAppRuntime` 的当前 `screen` 只由 NavigationContainer 的 route change 回调更新，页面跳转只能调用 `appNavigation` 的导航命令。
