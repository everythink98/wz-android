...
`WRITE-01/02/04/05`：共享图片 NodeView 独占预览加载状态。编辑预填、源码解析与上传插入的图片在请求未完成时显示有高度的加载占位，失败时提供单张重试；重试保留原 URL，不重载编辑器。加载状态不进入 Markdown 或撤销历史，源码往返保留已加载节点，旧图片回调和销毁节点不能改变当前预览。canonical owner 为 `src/ui/composer/editorRuntime.test.ts`；设备检查正文已就绪而图片慢到的空白场景。
