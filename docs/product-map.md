...
| `SEARCH-01`、`SEARCH-02`、`SEARCH-03` | `src/searchFilters.test.ts`、`src/localSources.test.ts`、`src/app/useSearchController.test.ts`、`src/searchListItems.test.ts`、`tests/ui/search-screen.test.tsx` 和 `tests/ui/search-controller-ai.test.tsx` 固定预览/单站、筛选、分页、历史、AI 及每来源 `data/empty/partial/error/auth`；`tests/device/search-multi-source.ad` 与未登录 Replay 各只提交一次聚合搜索，逐来源等待当前请求 outcome，清空后检查来源和筛选 UI；`REG-TEST-002` 禁止旧完成标记冒充当前结果 | 各站当天是否有结果、首条可打开性、真实候选和 Search → Topic 返回由 Agent Live 独立报告；AI 零结果或当前不可用不得误判普通搜索失败。 |
| `SEARCH-04` | `REG-SEARCH-002`、`REG-SEARCH-007/008` 与 `REG-SOURCE-002` 由 `src/searchListItems.test.ts`、`tests/ui/search-screen.test.tsx` 和 `tests/ui/search-controller-ai.test.tsx` 固定 data/partial/首屏错误、旧结果保留、失败 cursor、解析空及显式恢复；`REG-SEARCH-014/015` 由安全、诊断和 hidden WebView UI 测试固定 Google flow-bound gate/result 与 SearchGuard 分类；未登录 Replay 固定四站权威身份、五站当前请求 outcome 和 relaunch 状态 | 受限、验证和 fallback 必须以 App 内会话为事实；外部数据可得性由 Agent Live 报告，不能靠隐藏结果、自动重试或错误态冒充成功。 |
...
| `RELEASE-01`、`RELEASE-02` | `REG-OPS-006` 至 `REG-OPS-011` 与 `REG-TEST-002` 至 `REG-TEST-007` 由 `src/androidSmokeGuard.test.ts` 固定六文件/六次普通 relaunch、两次未登录 relaunch、零 retry、无固定 sleep/坐标/写入、非 Feed 旅程不依赖聚合 Feed、动态 Replay 使用当前请求 outcome、NodeSeek App-owned settled、四站权威未登录身份和小隐寺共同结算 oracle；`scripts/smoke-android.mjs` 只以 `main-tab-feed`、前台包名和无崩溃日志输出 `APK_SANITY`，Replay 独立输出 `DEVICE_REPLAY_PASS` | 只有明确发布任务运行正式签名与上传流程；两种证据都不证明第三方当前有数据。 |
