...
| `TOPIC-04` | 详情提供本机收藏；主题菜单提供分享、仅刷新评论、完整刷新、阅读设置和原站打开；操作后保持当前详情上下文。 | `src/screens/topic/TopicMenu.tsx`、`src/app/useTopicSessionController.ts`、`src/app/useReaderDataController.ts` | `src/app/useTopicSessionController.test.ts`、`src/app/useReaderDataController.test.ts`、`tests/ui/topic-and-more-controls.test.tsx`、`tests/ui/topic-reply-filters.test.tsx`、`tests/ui/app-navigator.test.tsx` | 打开右上菜单；分享后取消，按授权检查可恢复的本机收藏；阅读设置返回回归见 `REG-TOPIC-002`。 |
...
| `WRITE-03` | NodeSeek 支持点赞、鸡腿、反对、原站收藏和投票；linux.do 支持点赞、原站书签和投票；妖火支持可切换的原站收藏和投票：主题加载时从收藏列表按帖子 id 恢复状态，收藏沿用原站一次 GET 并从同源收藏夹响应取得收藏记录 id，取消收藏使用该记录 id 的一次 POST，只有服务端确认后才局部切换黄色收藏样式，不进入整页忙碌态，也不重新提交正文、图片或回复列表，见 `REG-WRITE-002`、`REG-WRITE-003`、`REG-WRITE-004`、`REG-WRITE-005`；V2EX 只展示互动信息。首次投票成功后本地所选项票数与参与人数只增量一次。不可逆或客户端不能撤销的操作不得按“可恢复切换”验收。 | `src/screens/topic/TopicActionBar.tsx`、`src/screens/topic/TopicPolls.tsx`、`src/app/useTopicActionsController.ts`、`src/app/useHtmlRenderingController.tsx`、`src/app/AppRoot.tsx`、`src/yaohuoApi.ts` | `src/topicActionState.test.ts`、`src/screens/topic/TopicPolls.test.ts`、`src/app/useTopicActionsController.test.ts`、`tests/ui/topic-reply-filters.test.tsx`、三站 action client 测试 | 先看入口和权限；真实互动按具体站点、对象、可逆性取得授权并核对最终状态；正文稳定性按 `LIVE-WRITE-03` 用 30 fps 录屏逐帧确认。 |
...
| `TOPIC-02`、`TOPIC-04` | `tests/ui/topic-components.test.tsx`、`tests/ui/topic-and-more-controls.test.tsx`、`tests/ui/image-preview.test.tsx`、`tests/ui/topic-reply-filters.test.tsx`、`tests/ui/app-navigator.test.tsx`；Replay 只证明详情可达，图片和分享仍走专项路径 | `REG-TOPIC-002` 的普通 UI 测试固定阅读设置作为 Topic 临时子页打开，返回后原 Topic 内部状态仍保留，计作该行为的 `UI_PASS`；保存图片、收藏切换和系统分享按授权并恢复。 |
...
| `WRITE-01`、`WRITE-02`、`WRITE-03`、`WRITE-04` | `tests/ui/reply-composer.test.tsx` 固定楼层回复、编辑、三站表情和上传入口；`tests/ui/topic-components.test.tsx` 固定逐站回复权限、互动入口和投票约束；请求由 Vitest 固定；`REG-WRITE-001` 固定首次投票计数且重复补丁不重计，`REG-WRITE-002` 固定妖火收藏单次 GET、参数和同源收藏夹重定向 oracle，`REG-WRITE-003` 固定收藏记录恢复、取消 POST、确认后状态补丁和黄色取消样式，`REG-WRITE-004` 固定收藏只走局部补丁且不切换全局 busy，`REG-WRITE-005` 固定 bookmark-only 更新不重建 HTML 渲染输入，设备录屏再固定正文不重绘 | 默认 Replay 不提交评论或写入；获授权操作按 `tests/live/agent-live.md` 逐项记录可逆性、恢复和残留。 |
...
| [妖火收藏无法取消且页面不显示已收藏](regression-corpus.md#reg-write-003-妖火收藏无法取消且页面不显示已收藏) | `WRITE-03` | 收藏列表的帖子 id 与 `data-fav-id` 收藏记录 id 映射、主题加载状态、收藏成功响应解析、取消收藏 POST、服务端确认后本地状态补丁和黄色激活样式；真实验收必须恢复初始收藏状态。 |
