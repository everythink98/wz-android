...
| `FEED-02` | 首页可切换所有已启用来源；每个单站只展示该站可用分类，切换完成后从目标列表首项开始。一级来源、唯一固定的二级导航、目标 Loading 与读取只消费同一个 `feedSource`：横滑期间保持原来源，由 Android Compose fling delegate 持续记录最新目标，delegate 成功完成后通过 `onIndexChange` 只提交最终非起点目标一次；协程取消或最终返回起点零提交，途中反向只取最终目标。点击立即切换且不播放 Pager 点击动画。Feed 不从 fractional position、idle 或额外视觉状态推导选择，也不建立导航覆盖层。切换期间只显示目标 Loading，不回显旧来源或空列表；每次完成切换重新读取目标来源，刷新失败保留可信列表和分页位置。公开来源继续可读，需要身份的来源显示可恢复状态。 | `src/features/feed/FeedScreen.tsx`、`src/features/feed/useFeedController.ts`、`src/sources/feedRead.ts`、`src/sources/readGateway.ts` | `tests/ui/feed/feed-screen.test.tsx`、`tests/ui/feed/feed-controller-session.test.tsx`、`src/sources/feedRead.test.ts`、`src/sources/readGatewayContract.test.ts` | 逐站滑动、点选、取消和连续反向；确认最终目标提交时两级导航、Loading 与读取同步，取消零读取，成功切换只读取一次并从首项开始。 |
...
`TOPIC-01/02/03`、`NAV-02/03`：四站 inline 图片、Emoji 和 GIF 在文字流中保持正确尺寸、基线和占位；系统字体缩放可以改变文字度量，但不得改变 inline View 以 DIP 测量的占位宽高，DIP 到物理像素必须向上取整，不能因小字体缩窄占位而让真实图片覆盖相邻文字。同一图片只有一个 Native 加载 owner，加载、进度、失败、取消和显式重试必须有限结算，旧请求事件不得覆盖当前内容。正文媒体同时运行最多 4 个，不按帖子、站点或图片数量特判。
...
| --- | --- | --- | --- |
| 关联 `FEED-02` | 首页来源：全部、V2EX、linux.do、NodeSeek、妖火。 | 默认“全部”；一级来源、唯一固定的二级导航、Loading 与读取只消费同一个 `feedSource`。拖动期间保持原来源；native fling delegate 成功完成后只对最终非起点目标通过一次 `onIndexChange` 清空分类、删除目标来源所有 Feed Query 变体并读取。点击直接提交且不播放 Pager 动画；inactive scene 永远 Loading，取消或最终返回起点零请求，途中反向只提交最终目标。各站排序值彼此独立保留，只有目标提交后才显示该站已保存排序；不能把上一个或目标温缓存的列表、可视位置、错误或分页 cursor 带入。 | `tests/ui/feed/feed-screen.test.tsx`、`tests/ui/feed/feed-controller-session.test.tsx`、`tests/integration/query-session-contracts.test.ts`；逐站切换、取消并确认最终目标提交时两级导航、Loading 与读取同步，请求次数、旧标题不可见与首项。 |
