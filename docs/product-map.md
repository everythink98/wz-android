...
| ID | 用户入口与行为契约 | 主要代码入口 | 自动测试 | 模拟器路径 |
| --- | --- | --- | --- | --- |
| `SEARCH-01` | “全部”并行搜索四站，按来源分组；条目展示来源、标题、作者和头像，局部失败可重试，成功来源继续展示。 | `src/screens/SearchScreen.tsx`、`src/app/useSearchController.ts`、`src/sources/sourceGateway.ts`、`src/localLinuxdo.ts`、`src/components/TopicCard.tsx` | `src/app/useSearchController.test.ts`、`src/sourceErrors.test.ts`、`src/searchListItems.test.ts`、`src/localSources.test.ts` | 搜索 → 全部 → 提交同一关键词；检查各来源作者和头像并打开结果。 |
| `SEARCH-02` | 可切换四个单站；单站与“全部”分组都在用户向下滚动、分页哨兵可见后自动续页，每次滚动最多加载一页，“全部”同一时刻只加载一个来源。最近搜索只填充关键词，提交后才请求，历史最多 20 条且可逐条删除；清空关键词会清空当前结果，“全部”结果组可展开/收起。linux.do 的普通结果独立分页；开启 AI 搜索后仍以普通结果顺序为准，只在末尾追加去重后的 AI 独有结果。 | `src/screens/SearchScreen.tsx`、`src/app/useSearchController.ts`、`src/searchHistory.ts`、`src/searchListItems.ts`、`src/searchControllerResults.ts` | `src/app/useSearchController.test.ts`、`src/searchHistory.test.ts`、`src/searchListItems.test.ts`、`tests/ui/search-screen.test.tsx`、`tests/ui/search-controller-ai.test.tsx` | 单站和“全部”搜索后向下滚动自动续页，确认一次滚动只加载一个来源；点历史后再提交、删除历史并展开/收起结果组；linux.do 开关 AI 后继续加载普通结果。 |
...
| `ACCOUNT-02` | 登录/验证必须在 App 内 WebView 完成；Cookie bridge、generation 和 fallback 不能让旧会话覆盖新状态。NodeSeek 设备 Replay 必须直接由 WebView 可访问树中的真实页面内容证明页面可用，不能把弹层已打开、`onLoadEnd` 或内部 ready testID 当作页面可用；错误与 readiness 互斥由 `REG-NODESEEK-002` 的 UI 测试固定，设备判据见 `REG-NODESEEK-003`。清除某站登录只影响该站网站会话。 | `src/app/useAccountController.ts`、`src/app/useSessionController.ts`、`src/app/GlobalModalHost.tsx`、`src/app/LinuxDoVerifyModal.tsx`、`src/components/LoginWebViewModal.tsx`、`src/screens/more/MorePanels.tsx`、`src/loginWebViewScripts.ts` | `src/app/sessionControllerHelpers.test.ts`、`src/cookieCleanup.test.ts`、`src/yaohuoCookies.test.ts`、`src/androidSmokeGuard.test.ts`、`tests/ui/account-site-panels.test.tsx` | 分别打开三站 App 内登录/验证页；NodeSeek 等待真实 WebView 标题、logo 和正文入口后返回；禁止用浏览器代替，清除登录需明确授权。 |
...
| 关联 `SEARCH-02` | 关键词输入/清空、提交、最近搜索逐条删除；“全部”结果按站点分组并可展开/收起，每站独立重试和自动分页。 | 空关键词不请求；点历史只填入；输入与已提交词不一致时旧结果立即失效；分页使用已提交词而不是正在编辑的词。自动分页只有用户滚动后才能 arm，一次滚动最多一页，“全部”不并发续页。 | `src/searchHistory.test.ts`、`src/searchListItems.test.ts`、`src/app/useSearchController.test.ts`、`tests/ui/search-screen.test.tsx`、`tests/ui/search-controller-ai.test.tsx`；检查删除、折叠、局部错误、自动分页和 `REG-SEARCH-002`。 |
...
| `SEARCH-01`、`SEARCH-02`、`SEARCH-03` | `src/searchFilters.test.ts`、`src/localSources.test.ts`、`src/app/useSearchController.test.ts`、`src/searchListItems.test.ts`、`tests/ui/search-screen.test.tsx`、`tests/ui/search-controller-ai.test.tsx`、`tests/device/search-topic-user-return.ad`、`tests/device/search-multi-source.ad`；V2EX 覆盖筛选、详情和逐层返回，linux.do 覆盖候选筛选、AI 并发/缓存/去重、普通分页与返回，NodeSeek、妖火覆盖独立查询完成、来源隔离、首条结果打开与返回；`REG-SEARCH-002` 固定自动续页的滚动门禁和分页错误不隐藏旧结果，`REG-SEARCH-003` 固定 linux.do 标准搜索 post 的作者和头像映射 | 动态结果只断言可打开、来源和状态，不固定标题或数量；AI 零结果或当前不可用不得误判普通搜索失败。 |
| `SEARCH-04` | `REG-SEARCH-002` 由 `src/searchListItems.test.ts`、`tests/ui/search-screen.test.tsx` 和 `tests/ui/search-controller-ai.test.tsx` 固定首屏/分页错误分流、旧结果保留、失败 cursor 及普通/验证回调原页重试；`REG-NODESEEK-002` 由 `tests/ui/account-site-panels.test.tsx` 固定错误状态不暴露 ready；`REG-NODESEEK-003` 由 `src/androidSmokeGuard.test.ts` 禁止退回内部 marker，`tests/device/nodeseek-session.ad` 直接等待真实 WebView 标题、logo 和正文入口 | 受限、验证和 fallback 必须以 App 内会话为事实；动态分页失败不靠隐藏结果或自动重试掩盖。 |
...
| `ACCOUNT-01`、`ACCOUNT-02`、`ACCOUNT-03` | `tests/ui/account-site-panels.test.tsx` 固定三站登录 WebView 的加载、失败、刷新、关闭及 `REG-NODESEEK-002` 错误/readiness 互斥；`src/androidSmokeGuard.test.ts` 与 `tests/device/nodeseek-session.ad` 固定 `REG-NODESEEK-003` 的真实页面判据；`tests/device/more-readonly.ad` 固定三站已登录状态和页签切换 | 三站登录、Cookie 和 WebView 的最终状态只接受 App 内证据；不清登录。 |
...
| [搜索分页失败隐藏已有结果](regression-corpus.md#reg-search-002-分页失败隐藏已有结果) | `SEARCH-01`、`SEARCH-02`、`SEARCH-04` | 搜索列表构建、分页哨兵门禁、来源串行、失败页重试路由；单站与“全部”分别检查已加载结果、尾部错误和原页重试。 |
