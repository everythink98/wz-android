...
| 关联 `ACCOUNT-04`、`WRITE-04` | `REG-ACCOUNT-010` | NodeImage phase URL、安全随机 nonce（Web Crypto 或现有 Native `SecureRandom`）、局部 payload、owner/epoch/final reconcile、凭据 generation、无全局注入和 action controller 测试。 |
...
| `ACCOUNT-01`、`ACCOUNT-02`、`ACCOUNT-03`、`ACCOUNT-04` | `REG-LINUXDO-001/002/003` 由 `src/app/useVerificationController.test.ts`、四类带 `QueryClientProvider` 的 read controller RNTL、`src/app/sessionControllerHelpers.test.ts`、`src/nodeseekBrowserFetchScript.test.ts` 和 `tests/ui/hidden-browser-host.test.tsx` 固定；`REG-ACCOUNT-031/035` 由 `src/authSurfaceCoordinator.test.ts`、`src/managedCookies.test.ts`、三站 verifier、`tests/ui/account-status-controller.test.tsx` 与 Feed/Search/Topic/User consumer RNTL 固定 surface 矩阵、三态身份、同步 runtime commit、关闭后恢复、原子 epoch 提交和 barrier；`REG-ACCOUNT-010` 另由 NodeImage flow/script/credential 与 modal 测试固定 owner/epoch/nonce/精确文档注入。`tests/ui/topic-actions-controller.test.tsx` 的 `REG-WRITE-022/024` 固定只有 typed identity evidence 请求 Account Query 对账，ordinary/permission 不建立 barrier；既有 Account 编号继续固定失败隔离、Query 唯一投影、origin、摘要和显式清理；`tests/ui/account-site-panels.test.tsx` 固定原三站登录 WebView 的加载、失败、刷新、关闭及 readiness；`src/androidSmokeGuard.test.ts` 与 `tests/device/nodeseek-session.ad` 固定真实页面判据 | 原三站登录、Cookie 和 WebView 的最终状态只接受 App 内证据；linux.do challenge 只自然触发，不清登录。A→B、真实退出、真实 NodeImage 授权与 Cookie clear 未授权时分别记 `NOT_VERIFIED`。 |
