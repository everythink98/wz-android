...
| ID | 用户入口与行为契约 | 主要代码入口 | 自动测试 | 模拟器路径 |
| --- | --- | --- | --- | --- |
| `ACCOUNT-01` | 更多 → 账号中心统一显示 NodeSeek、linux.do、妖火、小隐寺状态；公共刷新由四个独立 Query 一次更新四个可登录来源，远端 data/error/busy 直接从 Query result 派生，不回写第二份 session projection；小隐寺只读账号检查也不得借授权 workflow event 回写再投影，见 `REG-ACCOUNT-016`，普通检查失败必须保留上一份可信 Query 身份并只显示该站错误，见 `REG-ACCOUNT-017`。保存、清除、换号和重新授权等外部会话变化必须 reset 目标站 active/disabled Account Query；查询自身确认失效时先提交 exact `expired`，再只 seed 该结果到新 generation key，普通新 scope 不得继承旧登录；其他三站 data/error/busy 保持不变，见 `REG-ACCOUNT-019`。Account Query 与当前验证 workflow 合并后的同一 view model 是账号页、搜索登录路径/AI 和 Topic 写入口的唯一会话权限来源，不能让各入口分别读取 workflow 或 Cookie 猜测，见 `REG-WRITE-016`、`REG-LINUXDO-005`。未登录、匿名、验证、授权中、失效和检查失败不得混淆。单站凭据、启动恢复、身份、摘要或刷新收尾失败只标记该站、保留可信状态并允许其他站完成，见 `REG-ACCOUNT-001/002/003/006/008/017/019`；刷新期间保存的新会话不能被旧 generation 的结果覆盖，见 `REG-ACCOUNT-009`；妖火和 linux.do 明确失效不得被普通清理或 Cookie 状态覆盖，见 `REG-ACCOUNT-004/008`。四站只有当前凭据返回 current user 或可信账号容器内的 self-account 结构才算登录；明确匿名状态码/字段或准确游客结构才算退出，其他响应均 unknown。Cookie、旧 ID、Topic 作者、公开资料和普通业务文案都不是身份正证据；NodeSeek 当前游客结构优先于残留用户字段；linux.do 不得用匿名 CSRF 或残留 `_t` 证明登录；妖火公开内容里的“我的/欢迎”不得证明登录；小隐寺只以 User API `/session/current.json` 为准。冷启动凭据只能建立候选态，未知检查只保留此前远端已确认的数据，不能保留本次 Cookie 猜测。已登录且已识别身份时可进入本人主页；小隐寺的重新授权与撤销授权仍由独立授权面板提供。 | `src/screens/more/AccountCenterPanel.tsx`、`src/app/useAccountStatusController.ts`、`src/app/useSessionController.ts`、`src/app/useAccountCredentialController.ts`、`src/app/useXiaoyinsiAuthController.ts`、`src/siteSessionState.ts` | `src/screens/more/accountCenter.test.ts`、`src/app/accountStatusHelpers.test.ts`、`tests/ui/account-status-controller.test.tsx`、`src/linuxdoActionClient.test.ts`、`src/app/sessionControllerHelpers.test.ts`、`src/app/serverState.test.ts`、`tests/ui/account-controller.test.tsx`、`src/app/accountCredentialDiagnostics.test.ts`、`src/siteSessionState.test.ts`、`tests/ui/search-controller-ai.test.tsx`、`tests/ui/xiaoyinsi-auth-controller.test.tsx`、`tests/ui/topic-actions-controller.test.tsx` | 更多 → 账号中心 → 刷新账号状态；逐站核对状态；保留自然残留 Cookie 冷启动时核对四站账号、搜索与写入口一致，已登录小隐寺进入本人主页后返回。 |
| `ACCOUNT-02` | NodeSeek、linux.do、妖火登录/验证必须在 App 内 WebView 完成；Cookie bridge、generation 和 fallback 不能让旧会话覆盖新状态，桥接消息还必须来自对应站点的 HTTPS host，见 `REG-ACCOUNT-003/004/005/009`。是否需要面板必须由 `cf-mitigated: challenge` 或严格验证页 DOM/script 证明；普通 403/429、登录接口 404、业务 JSON、可读页面和正文讨论不得改变会话为 verifying，见 `REG-VERIFICATION-002`。可见 WebView 使用 Android provider 的真实默认 UA，见 `REG-VERIFICATION-003`。原站 WebView Cookie 由网站与 Android `CookieManager` 持有；React Native、Expo Image 和 Expo Video 只按准确 URL 实时读取，不使用手工白名单或持久化 Cookie header，响应不得回写，见 `REG-ACCOUNT-026/027/029`、`REG-TOPIC-019`。App 候选只服务于 verifier、generation、CSRF、sid、touserid 等协议；四站只接受 current-user、明确游客或 unknown 三态。NodeSeek/linux.do 手动 probe 和妖火 self-account reader 明确 `logged-out` 时只更新 App 投影，`unknown` 保留可信身份；公开资料只补全。linux.do 恢复由 `REG-LINUXDO-001/002/003` 固定。隐藏 NodeSeek/linux.do fallback 首跳只传 URL，不附加 SecureStore Cookie header，并按 `REG-SOURCE-005/006` 稳定排队。只有用户明确点击“清除登录”才允许定向过期并回读确认；NodeSeek 保留 CF Cookie，见 `REG-ACCOUNT-007/022/026`。原生桥只使用公开 CookieManager API，不读取私有数据库。页面事件保持被动，见 `REG-VERIFICATION-001`。清除某站登录只影响该站。 | `src/androidWebViewUserAgent.android.ts`、`plugins/withNetworkProxyModule.js`、`plugins/withLinuxDoCookieModule.js`、`src/yaohuoCookieBridge.ts`、`src/app/useAccountController.ts`、`src/app/useSessionController.ts`、`src/app/sessionControllerHelpers.ts`、`src/app/HiddenBrowserHost.tsx`、`src/app/GlobalModalHost.tsx`、`src/app/LinuxDoVerifyModal.tsx`、`src/components/LoginWebViewModal.tsx`、`src/loginWebViewScripts.ts`、`src/yaohuoApi.ts` | `src/request.test.ts`、`src/releasePackaging.test.ts`、生成的 `NetworkProxyRuntimeTest.kt`、`tests/ui/account-controller.test.tsx`、`src/app/useVerificationController.test.ts`、`src/app/sessionControllerHelpers.test.ts`、`tests/ui/topic-actions-controller.test.tsx`、`src/linuxdoCf.test.ts`、`src/yaohuoApi.test.ts`、`src/yaohuoActionClient.test.ts`、`src/nodeseekBrowserFetchScript.test.ts`、`tests/ui/hidden-browser-host.test.tsx`、`src/cookieCleanup.test.ts`、`tests/ui/account-site-panels.test.tsx` | 分别打开原三站 App 内登录/验证页；NodeSeek 等待真实 WebView 标题、logo 和正文入口后返回；linux.do 只用自然 challenge 验收且不清 Cookie；这三站禁止用外部浏览器代替，清除登录需明确授权。 |
...
`ACCOUNT-01`、`SEARCH-04`、`TOPIC-01`、`WRITE-01/03` 共享登录投影 seam：普通页面读取凭据只能发布不带身份结论的观察事件，不得把账号检测确认的登录降级；明确的当前账号验证结果仍按站独立生效，见 `REG-ACCOUNT-023`。NodeSeek 当前身份只读当前首页/设置页的 `__config__.user` 或专属 self-account 结构，不调用不存在的无 ID `getInfo` 路由，也不把 `/api/account/getInfo/{id}` 公开资料当作登录证明；页面准确游客控件只能授权 App 投影为失效，不能授权删除原站 Cookie。清除登录是独立的用户破坏性操作，见 `REG-ACCOUNT-024/026`。

`ACCOUNT-01/02` 补充当前身份接口门禁：生产 endpoint 必须来自官方源码/文档、当前站点实际调用或成熟客户端，测试 mock 不能创造接口或状态码契约。四站分别以 NodeSeek 当前页 `__config__.user`/本人控件、linux.do Cookie session current user、小隐寺 User API session current user、妖火 WAP `div.top2` 本人导航作为登录正证据；NodeSeek 不递归接受无关嵌入 profile。Cookie 名只用于摘要，不能直接证明登录，也不能阻止已有候选进入真实 current-session 验证；adapter 无 current user 不能保存。退出证据按站点协议分别判断，未获契约支持的状态一律 unknown。小隐寺只有 reader 识别 Discourse JSON `invalid_access` 后产生的 typed expiry 可以退出，Controller 不解释原始 403；妖火公开资料只补全已证明身份并用真实昵称替换数字 ID 占位，补全失败不得退出或清理。见 `REG-ACCOUNT-025/026`。
...
| `MORE-05` | 开发版测试工具只临时模拟四个可登录来源匿名，不删除 Cookie 或小隐寺授权，重启失效；账号 Query 的登录结果不得覆盖搜索、诊断和写入入口的临时匿名 view model，见 `REG-TEST-003`；正式版不显示。 | `src/screens/MoreScreen.tsx`、`src/app/AppRoot.tsx`、`src/app/useSessionController.ts`、`src/siteSessionState.ts` | `src/androidFeatureHelpers.test.ts`、`src/siteSessionState.test.ts`、`src/androidSmokeGuard.test.ts`、`tests/device/anonymous-readonly.ad` | 当前开发包 → 更多 → 测试工具；先确认真实账号仍登录，再检查匿名搜索提示、结果与首页，relaunch 后确认恢复；完整匿名 Replay 通过后再跑不含该 dev-only 文件的 Release Smoke。 |
...
| `RELEASE-02` | 发布候选覆盖安装到指定保留登录态设备；覆盖安装后的第一次启动必须从启动前设备 epoch 与 marker 起进入有界包级日志窗口，启动与日志只形成 `APK_SANITY`；七条 release-safe `.ad` 旅程形成发布包的 `DEVICE_REPLAY_PASS`，依赖 `__DEV__` 的匿名旅程由同 revision 开发包独立证明，见 `REG-OPS-009`。Replay 要求 `agent-device >= 0.19.0`，不自行关闭 session，由 test harness 先停止录屏再 cleanup；runner 只恢复当前唯一 session/device manifest 归属的录屏，active manifest、对应 `.tmp`、工具 MP4 或 recorder 只要存在就阻断，未知现场不得清理；机器可读 capture 只解析 stdout，不把 stderr 诊断拼入 JSON；每条 tracked Replay 自己声明 wall-clock budget，runner 不覆盖，见 `REG-OPS-007`、`REG-OPS-008`、`REG-OPS-010`、`REG-OPS-011`。 | `scripts/agent-device-runtime.mjs`、`scripts/smoke-android.mjs`、`scripts/run-device-replay.mjs`、`scripts/release-android.mjs`、`tests/device/*.ad` | `src/androidSmokeGuard.test.ts`、`src/releasePackaging.test.ts` | 当前开发包先跑完整八条；随后指定 `WZ_ANDROID_SMOKE_DEVICE` 执行 APK sanity 与七条 release-safe Replay。不得清数据、恢复旧 APK、清理未知录屏或上传 smoke APK。 |
...
| 关联 `TOPIC-02`、`ACCOUNT-01` | `REG-TOPIC-019` | 原生 Cookie handler、RN/Fresco/Expo Image 同 client、Expo Video、preview/modal/controller/save 测试：媒体按准确 URL 实时读取，异常不匿名回退，响应不写回且不保留 Cookie 快照。 |
...
| 关联 `ACCOUNT-02` | `REG-ACCOUNT-022` | `src/cookieCleanup.test.ts` 固定 NodeSeek host/domain Cookie 身份与删除后回验；`tests/ui/account-site-panels.test.tsx` 固定 NodeSeek/妖火新凭据 attempt 只向原 WebView 注入，不销毁页面。 |
...
| `FEED-01`、`FEED-04` | `tests/ui/feed-screen.test.tsx`、`tests/ui/topic-card.test.tsx`、`tests/ui/feed-controller-xiaoyinsi.test.tsx` 固定 `REG-SOURCE-002` 的解析空与失败分页不落地；`tests/device/feed-topic-return.ad` 固定普通会话返回链，`tests/device/anonymous-readonly.ad` 固定临时匿名时“全部”及 NodeSeek、linux.do、小隐寺仍有真实首条，妖火进入登录限制 | 五站当天内容、分页和局部失败继续做 Live 验收。 |
...
| `SEARCH-01`、`SEARCH-02`、`SEARCH-03` | `src/searchFilters.test.ts`、`src/localSources.test.ts`、`src/app/useSearchController.test.ts`、`src/searchListItems.test.ts`、`tests/ui/search-screen.test.tsx`、`tests/ui/search-controller-ai.test.tsx`、`tests/device/search-topic-user-return.ad`、`tests/device/search-multi-source.ad`、`tests/device/anonymous-readonly.ad`；“全部”覆盖固定预览和单站进入，V2EX 覆盖筛选、详情和逐层返回，linux.do 覆盖历史立即提交、候选筛选、AI 并发/缓存/去重、普通分页与返回；临时匿名下先由 `REG-TEST-003` 固定 NodeSeek、linux.do 未登录提示，再要求 NodeSeek、linux.do、小隐寺返回首条，妖火进入登录限制；`REG-TEST-002` 固定成功路径直接等待结果且来源概览真实可点击，不误用残留完成标记；`REG-SEARCH-002` 固定单站自动续页的滚动门禁和分页错误不隐藏旧结果，`REG-SOURCE-002` 固定解析空失败页保留 cursor，`REG-SEARCH-003` 固定 linux.do 首帖作者和头像映射，`REG-SEARCH-013` 固定两站回复命中保留且不冒充楼主 | 动态结果只断言可打开、来源和状态，不固定标题或数量；AI 零结果或当前不可用不得误判普通搜索失败。 |
| `SEARCH-04` | `REG-SEARCH-002`、`REG-SEARCH-007/008` 与 `REG-SOURCE-002` 由 `src/searchListItems.test.ts`、`tests/ui/search-screen.test.tsx` 和 `tests/ui/search-controller-ai.test.tsx` 固定首屏/分页错误分流、旧结果保留、失败 cursor、解析空、聚合不自动弹面板及普通/验证回调原页重试；`tests/device/anonymous-readonly.ad` 固定临时匿名不会偷用 Cookie、公开 fallback 可搜索、妖火收口到 App 内登录面板且 relaunch 恢复原会话；`REG-LINUXDO-001/002/003` 固定验证恢复，`REG-NODESEEK-002/003` 固定真实 WebView readiness | 受限、验证和 fallback 必须以 App 内会话为事实；动态分页失败不靠隐藏结果或自动重试掩盖。 |
...
| `MORE-01`、`MORE-02`、`MORE-03`、`MORE-04`、`MORE-05` | `src/networkProxyControllerGuard.test.ts` 固定 `REG-PROXY-001` 的加载失败门禁；`tests/ui/network-proxy-modal.test.tsx` 固定代理校验、选择、编辑、连通性测试、密码遮蔽、启停和删除确认；生成的 Kotlin JUnit 与 `src/releasePackaging.test.ts` 固定 `REG-PROXY-004/005`；`tests/ui/more-screen.test.tsx` 固定诊断、更新、备份和开发工具状态；`tests/ui/topic-and-more-controls.test.tsx` 固定外观选择；`tests/device/more-readonly.ad` 只读覆盖代理 Modal、诊断、备份和外观入口 | 代理原生恢复、真实公网代理、系统分享、文件选择、安装器和开发工具重启后的最终状态必须记录；未经授权不启用代理。 |
| `RELEASE-01`、`RELEASE-02` | `REG-OPS-006` 至 `REG-OPS-011` 与 `REG-TEST-002/003` 由 `src/androidSmokeGuard.test.ts` 固定 Replay 不自行 close、录屏文件存在性门禁、最低工具版本、dev-only/Release 分流、CLI stdout/stderr 分离、每文件超时预算、搜索成功等待 oracle 及匿名状态提示；同文件固定首次安装后启动日志和录屏所有权边界；`scripts/smoke-android.mjs` 输出 `APK_SANITY`，Replay 输出 `DEVICE_REPLAY_PASS` | 只有明确发布任务运行正式签名与上传流程。 |
...
| [写操作确认失效后未更新统一会话投影](regression-corpus.md#reg-write-022-写操作确认失效后未更新统一会话投影) | `ACCOUNT-01`、`SEARCH-04`、`TOPIC-01`、`MORE-02`、`WRITE-01`、`WRITE-03` | NodeSeek/妖火 typed action expiry 经现有 Session dispatcher 关闭目标站写权限并失效对应 Query scope；妖火 verification 与 expired 分流，unknown/权限/stale 不改变身份，全部路径零 Cookie 清理。 |
...
| [Replay 设备 ID 与名称映射](regression-corpus.md#reg-ops-003-replay-把设备-id-当成设备名称) | `RELEASE-02` | 环境选择可使用 ID 或名称；ADB 身份校验必须使用解析后的 ID，`agent-device test` 必须使用解析后的显示名称；单元映射、开发包八条和发布包七条 `retries=0` Replay 都要通过。 |
| [AVD 名与 booted device 显示名映射](regression-corpus.md#reg-ops-004-avd-名与设备显示名不一致导致-replay-被拒绝) | `RELEASE-02` | Smoke 可用 AVD 名启动；Replay 设备发现只把下划线/空白差异视为同名且必须唯一，不能模糊选择其他设备；单元映射、APK sanity、开发包八条及发布包七条零重试 Replay 都要通过。 |
...
| [Replay 自行 close 导致录屏复活](regression-corpus.md#reg-ops-006-replay-自行关闭-session-导致录屏复活并丢失-manifest) | `RELEASE-02` | 八条 tracked Replay 都不执行 `close`；开发包完整执行八条，发布包执行七条 release-safe 文件，test harness 均先停止视频再清理 session，结束后 manifest、进程和 scratch 必须归零。 |
...
| [开发态匿名 Replay 进入 Release Smoke](regression-corpus.md#reg-ops-009-开发态匿名-replay-被错误放进-release-smoke) | `MORE-05`、`RELEASE-02` | 默认设备套件保留完整八条；Release Smoke 明确排除 `anonymous-readonly.ad` 并跑七条 release-safe 文件，不能向正式 APK 暴露测试工具。 |
...
| [NodeSeek 私有媒体在预览和保存时丢失实时会话](regression-corpus.md#reg-topic-019-nodeseek-私有媒体在预览和保存时丢失实时会话) | `TOPIC-02`、`ACCOUNT-01` | RN/Fresco、Expo Image、Expo Video、预览与保存均按准确 URL 实时读取只读 WebView Cookie；不保留媒体快照、不回写响应、不向非受管域发送。 |
