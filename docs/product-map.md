...
| `RELEASE-01`、`RELEASE-02` | `REG-OPS-006` 至 `REG-OPS-010` 与 `REG-TEST-002/003` 由 `src/androidSmokeGuard.test.ts` 固定 Replay 不自行 close、录屏文件存在性门禁、最低工具版本、dev-only/Release 分流、CLI stdout/stderr 分离、搜索成功等待 oracle 及匿名状态提示；同文件固定首次安装后启动日志和录屏所有权边界；`scripts/smoke-android.mjs` 输出 `APK_SANITY`，Replay 输出 `DEVICE_REPLAY_PASS` | 只有明确发布任务运行正式签名与上传流程。 |
