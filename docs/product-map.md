...
`TOPIC-01/02/03`、`NAV-02/03`：图片位置由作者 HTML 的节点顺序、`br`、块级父容器和继承的显式 `text-align` 决定；共享 compiler 把普通安全 `img` 保留为原锚点上的 textual 图片，浏览器式文字流自然决定同行或换行，只有明确的 figure/lightbox 媒体容器继续走块图。尺寸与位置解耦，显式尺寸或加载后的自然尺寸只能等比调整占用空间，不能重新分类位置；无作者对齐信号的块图在主楼、回复和引用中统一靠起始边。有界内联语义只接受 sanitizer 后由来源 adapter 写入的可信 marker；外部 `/face/` 仍是普通预览，不得按来源、角色、GIF、class、URL 或尺寸阈值猜位置。
