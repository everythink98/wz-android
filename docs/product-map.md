...
`TOPIC-01/02/03`：正文、原图、预览、保存、音频、视频、贴纸和卡片图共用媒体首跳 Referrer 规则；最终策略由元素、文档和真实 URL 关系决定，不能从站点名猜测。视频按固有比例显示；音频使用原生卡片且不自动播放，提供播放/暂停、时间、跳转和手动重试，离窗或 route inactive 时释放播放器，返回后保持暂停。错误或超时后等待用户重试，卸载后不再访问已释放资源；poster、暂停帧、附件语义和已显示图片在滚动或 Tab 切换后保持稳定。
...
| `TOPIC-02` | 正文正确呈现 HTML、链接、表格、代码、图片、附件、音频、视频、Emoji、贴纸、SVG 和站点格式；可信站内用户链接进入 App User route。块图先显示适屏版本并可渐进升级原图，失败时保留可读底图；预览支持相邻页、缩放、下拉关闭、重试、Back、无障碍翻页和按授权保存。安全 HTTP(S) 音频在主楼、回复、展开引用和采纳答案的原位置显示原生播放器，空源保留来源 fallback，危险源拒绝；播放器不自动播放，支持播放/暂停、时间、跳转、失败重试与无障碍进度操作，离窗或 route inactive 后停止并释放。inline 媒体保持文字基线，表格与代码可横滑且不破坏文字选择。所有媒体遵守来源 Referrer、身份隔离、取消、有限并发和有限重试；不可信 URL、恶意或超大正文必须有限结算，安全文字不得被静默删除。 | `src/features/topic/components/TopicContentList.tsx`、`src/features/topic/components/TopicContentBlock.tsx`、`src/features/topic/rendering/useHtmlRenderingController.tsx`、`src/platform/media/imagePreviewCatalog.ts`、`src/platform/media/mediaRequestContext.ts` | `src/domain/forum/topicContentSplit.test.ts`、`src/platform/media/mediaRequestContext.test.ts`、`tests/integration/html-sanitization-contracts.test.ts`、`tests/ui/topic/topic-image-loading.test.tsx`、`tests/ui/topic/image-preview.test.tsx`、`tests/ui/topic/topic-table-rendering.test.tsx` | 打开含表格、代码、图片、音频、视频和引用的四站主题，检查阅读、横滑/选择、音频控制、预览/返回、失败重试与页面位置；保存或其他写入需另获授权。 |
...
`TOPIC-02/03`：预览、引用、评论、查找、筛选和动作状态变化不得重载已显示的图片、Emoji、贴纸、音频、视频、链接卡片或 WebView；真实主题、会话或渲染配置变化时必须隔离旧内容。音频离窗或 route inactive 的显式回收不属于重载，返回后不得自动续播。
