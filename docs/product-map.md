...
`ACCOUNT-01/02/04/06` 共用 Account canonical identity 与 `ForumSessionEpochs`：A→A 只解除 barrier；A→B、A→anonymous 或 anonymous→B 原子提交新身份、递增目标站 epoch 并清理目标站及 `all` 私有 Query、Level/AI、Topic 服务端数据和 managed media 身份缓存；unknown 保留上一份可信身份与已加载内容只读。Feed/Search 聚合只暂停 dirty 来源，其他来源继续刷新；Topic route 的选择、滚动、筛选和草稿保留，但正文、回复和用户资料不能跨 epoch 复用。见 `REG-ACCOUNT-031`。
...
`WRITE-01/02/03/04` 与 NodeSeek 签到统一先经 `ensureWritableSession(source)` 取得一次性 identity/epoch ticket。dirty 会话先复核；换号、退出、unknown 或 ticket 过期均在 Query snapshot、optimistic update、文件选择、上传和 transport 前终止，文件选择与 Query 取消等待后再次校验。401/403 或 action 失败只触发一次 verifier，不自动重放非幂等请求。NodeImage 自动 Key 绑定 NodeSeek identity；旧版或手动 Key 首次跨身份使用前要求显式确认且不展示 Key。见 `REG-WRITE-023`。
...
| `ACCOUNT-01`、`ACCOUNT-02`、`ACCOUNT-03`、`ACCOUNT-04` | `REG-LINUXDO-001/002/003` 由 `src/app/useVerificationController.test.ts`、四类带 `QueryClientProvider` 的 read controller RNTL、`src/app/sessionControllerHelpers.test.ts`、`src/nodeseekBrowserFetchScript.test.ts` 和 `tests/ui/hidden-browser-host.test.tsx` 固定；`REG-ACCOUNT-031` 由 `src/authSurfaceCoordinator.test.ts`、`src/managedCookies.test.ts`、三站 verifier、`tests/ui/account-status-controller.test.tsx` 与 Feed/Search/Topic/User consumer RNTL 固定 surface 矩阵、三态身份、原子 epoch 提交和 barrier；既有 Account 编号继续固定失败隔离、Query 唯一投影、origin、摘要和显式清理；`tests/ui/account-site-panels.test.tsx` 固定原三站登录 WebView 的加载、失败、刷新、关闭及 readiness；`src/androidSmokeGuard.test.ts` 与 `tests/device/nodeseek-session.ad` 固定真实页面判据 | 原三站登录、Cookie 和 WebView 的最终状态只接受 App 内证据；linux.do challenge 只自然触发，不清登录。A→B、真实退出与 Cookie clear 未授权时分别记 `NOT_VERIFIED`。 |
...
| `WRITE-01`、`WRITE-02`、`WRITE-03`、`WRITE-04` | `src/writableSessionGate.test.ts` 与 `tests/ui/topic-actions-controller.test.tsx` 的 `REG-WRITE-023` 固定 clean fast-path、dirty 复核、ticket 过期、零 optimistic/picker/upload/transport 及非幂等不重放；`tests/ui/reply-composer.test.tsx` 固定楼层回复、编辑、表情和上传入口；`tests/ui/topic-components.test.tsx` 固定逐站权限、互动入口和投票约束；各站 action/request 测试与既有 `REG-WRITE-*` 保持消费者回归 | 默认 Replay 不提交评论或写入；获授权操作按 `tests/live/agent-live.md` 逐项记录可逆性、恢复和残留。 |
...
| [旧 Topic 或账号在身份待确认、换号后继续写入](regression-corpus.md#reg-write-023-旧主题或账号在身份待确认换号后继续写入) | `ACCOUNT-01`、`ACCOUNT-02`、`ACCOUNT-04`、`TOPIC-01`、`TOPIC-03`、`WRITE-*` | `ensureWritableSession` clean/dirty 路径、identity/epoch ticket、optimistic/snapshot/picker/upload/transport 前后门禁、NodeImage Key 归属及 401/403 只复核一次且不自动重放。 |
