...
describe('Discourse source action composition', () => {
  ...
  it('keeps Xiaoyinsi bookmark removal as a site operation override', () => {
    expect(buildDiscourseSourceActionRequest('xiaoyinsi', {
      type: 'set-bookmark',
      targetId: '42',
      targetType: 'Topic',
      active: false
  ...
  it('resolves upload results with registered site identity', () => {
    expect(discourseSourceUploadUrl('xiaoyinsi', { url: '/uploads/image.png' }))
      .toBe('https://forum.xiaoyinsi.com/uploads/image.png');
  });
});
