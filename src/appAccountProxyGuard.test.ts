...
describe('account proxy guard', () => {
  it('keeps the Yaohuo login check callback bound to the current proxy fetcher', () => {
    ...
    expect(source).toContain('}, [checkingRequestIdRef, clearYaohuoLoginState, currentYaohuoCredentialGeneration, forumFetchWithWebViewFallback, notify, saveYaohuoCookieHeader, setChecking, updateYaohuoSession]);');
  });
});
