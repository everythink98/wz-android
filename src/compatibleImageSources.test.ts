...
describe('compatible remote image sources', () => {
  it('[REG-TOPIC-038] preserves the original complex SVG document while producing a poster artifact', async () => {
    ...
    const fetcher = vi.fn<Fetcher>(async () => new Response(complexSvg, {
    ...
    expect(Buffer.from(String(artifact?.documentDataUri).split(',')[1] || '', 'base64').toString('utf8')).toBe(complexSvg);
  ...
  it('[REG-TOPIC-038] does not strip link elements or otherwise rewrite accepted SVG bytes', async () => {
    const svg = '\uFEFF<svg width="10" height="5"><a href="https://example.com/?value=1&gt;0"><text>linked</text></a></svg>\n';
    const artifact = await recoverCompatibleSvgArtifact({ uri: 'https://images.example.com/linked.svg' }, {
    ...
    expect(Buffer.from(String(artifact?.documentDataUri).split(',')[1] || '', 'base64')).toEqual(Buffer.from(svg, 'utf8'));
  });
  ...
  it('[REG-TOPIC-038] single-flights poster rendering and reuses the cached artifact with managed request headers intact', async () => {
    ...
    const fetcher = vi.fn<Fetcher>(async () => new Response('<svg width="10" height="5"></svg>', {
  ...
  it('[REG-TOPIC-038] rebuilds an evicted poster from the preserved SVG without another network request', async () => {
    ...
    expect((first.posterSource as ImageURISourceWithCacheKey).cacheKey)
      .not.toBe((original?.posterSource as ImageURISourceWithCacheKey).cacheKey);
    expect(fetcher).toHaveBeenCalledTimes(1);
  ...
  it('uses poster dimensions only when the SVG has no intrinsic dimensions', async () => {
    const artifact = await recoverCompatibleSvgArtifact({ uri: 'https://images.example.com/no-dimensions.svg' }, {
      fetcher: async () => new Response('<svg><rect width="10" height="5" /></svg>', {
        headers: { 'content-type': 'image/svg+xml' }
      }),
  ...
  it('derives a missing intrinsic dimension from the SVG viewBox aspect ratio', async () => {
    const artifact = await recoverCompatibleSvgArtifact({ uri: 'https://images.example.com/one-dimension.svg' }, {
      fetcher: async () => new Response('<svg width="100" viewBox="0 0 200 100"></svg>', {
        headers: { 'content-type': 'image/svg+xml' }
      }),
  ...
  it('[REG-TOPIC-038] ignores SVG-shaped comments before the real document root', async () => {
    ...
    const artifact = await recoverCompatibleSvgArtifact({ uri: 'https://images.example.com/root-comment.svg' }, {
      fetcher: async () => new Response(svg, { headers: { 'content-type': 'image/svg+xml' } }),
  ...
  it('[REG-TOPIC-038] uses the native validated document dimensions instead of decoding raw attributes twice', async () => {
    ...
    const artifact = await recoverCompatibleSvgArtifact({ uri: 'https://images.example.com/entity-dimensions.svg' }, {
      fetcher: async () => new Response(svg, { headers: { 'content-type': 'image/svg+xml' } }),
  ...
  it('rejects a non-SVG response without invoking the poster renderer', async () => {
    ...
    await expect(recoverCompatibleSvgArtifact(source, {
  ...
  it('[REG-TOPIC-038] bounds the complete recovery pipeline before downloads or base64 poster payloads accumulate', async () => {
    ...
    const recoveries = Array.from({ length: 32 }, (_, index) => recoverCompatibleSvgArtifact({
      uri: `https://images.example.com/bounded-${index}.svg`
    }, {
      fetcher,
      renderPoster: async () => poster(1, 1, `file:///cache/bounded-${index}.png`)
    }));
    const overflow = recoverCompatibleSvgArtifact({ uri: 'https://images.example.com/bounded-overflow.svg' }, {
      fetcher,
  ...
  it('[REG-TOPIC-038] gives a late download only the recovery deadline that remains', async () => {
    ...
    const posterResolvers: Array<(value: ReturnType<typeof poster>) => void> = [];
    ...
    try {
      const first = recoverCompatibleSvgArtifact({ uri: 'https://images.example.com/deadline-first.svg' }, {
        fetcher,
        renderPoster
      });
      const second = recoverCompatibleSvgArtifact({ uri: 'https://images.example.com/deadline-second.svg' }, {
        fetcher,
        renderPoster
      });
      const late = recoverCompatibleSvgArtifact({ uri: 'https://images.example.com/late-download.svg' }, {
        fetcher,
        renderPoster
      });
  ...
  it.each([
    ['advertised content length', new Response('', {
      headers: {
        'content-length': String(1024 * 1024 + 1),
        'content-type': 'image/svg+xml'
      }
    })],
    ['actual response bytes', new Response(`<svg>${'x'.repeat(1024 * 1024)}</svg>`, {
      headers: { 'content-type': 'image/svg+xml' }
    })]
  ...
  it('[REG-TOPIC-034] accepts an exact 1 MiB SVG without rewriting or rescanning its body', async () => {
    const svg = complexSvg.replace(
      '</svg>',
      `${' '.repeat(1024 * 1024 - Buffer.byteLength(complexSvg))}</svg>`
    );
    const renderPoster = vi.fn(async () => poster(1, 1, 'file:///cache/max.svg.png'));

    const artifact = await recoverCompatibleSvgArtifact({ uri: 'https://images.example.com/max.svg' }, {
      fetcher: async () => new Response(svg, { headers: { 'content-type': 'image/svg+xml' } }),
      renderPoster
    });
  ...
  it('[REG-TOPIC-034] rejects an oversized Blob before copying it into a JS ArrayBuffer', async () => {
    ...
    await expect(recoverCompatibleSvgArtifact({ uri: 'https://images.example.com/oversize-blob.svg' }, {
