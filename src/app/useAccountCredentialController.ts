...
export function useAccountCredentialController({
  ...
  const reloadCredentialSummaries = useCallback(async () => {
    ...
    const result = await loadCredentialSummariesWithTrace(
      (site) => credentialVault.getSummary(site),
      {
        generation,
        isCurrent: () => summaryLoadGenerationRef.current === generation
          && summaryRevisionRef.current === revision
      }
    );
    if (!result.ok) {
      if (!('stale' in result)) {
  ...
  const finishCredentialFillForLoginFailure = useCallback((
    ...
    if (!finishCredentialFillTraceForWebViewFailure(
      credentialFillTraceRef.current,
      site,
      attempt,
      reason
    )) {
  ...
  const handleCredentialLoginFormMessage = useCallback((event: WebViewMessageEvent) => {
    ...
    pendingCredentialFillSiteRef.current = null;
    setPendingCredentialFillSite(null);
    void credentialVault.readForFill(site)
      .then((credential) => {
  ...
  const handleAccountCenterCommand = useCallback(async (command: AccountCenterCommand) => {
    ...
    const trace = beginDiagnosticTrace('credential', 'delete', { site: command.site, store: 'secure-store', generation });
