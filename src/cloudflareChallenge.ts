...
function isCloudflareChallengeBody(body: string) {
  return (
    /<title\b[^>]*>[^<]*(?:just a moment|checking your browser|needs to review the security|attention required|enable javascript and cookies|请稍候|正在检查)[^<]*<\/title>/i.test(
      body
    ) ||
    /<h[12]\b[^>]*>\s*(?:just a moment|checking your browser|attention required|verify you are human|请稍候|正在检查|正在进行安全验证|请完成验证)[.!…\s]*<\/h[12]>/i.test(
      body
    ) ||
    /<(?:form|input|div|iframe|script)\b[^>]*(?:id|class|name|src|action)=["'][^"']*(?:cf-turnstile|cf-browser-verification|challenge-running|challenge-platform|cf_chl_|__cf_chl|challenge-error|challenges\.cloudflare\.com|\/cdn-cgi\/challenge)[^"']*["'][^>]*>/i.test(
      body
    ) ||
