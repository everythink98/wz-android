...
export function createUserListItems(tab: UserActivityTab, topics: Topic[], replies: UserReplyActivity[]): UserListItem[] {
  const seen = new Map<string, number>();
  if (tab === 'replies') {
    return replies.map((reply) => ({ type: 'reply', key: uniqueListKey(`${reply.source}:reply:${reply.id}`, seen), reply }));
  }
