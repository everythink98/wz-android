  ...
  return (
    <View style={styles.stack}>
      ...
      {waiting ? (
        <>
          <View style={styles.stack}>
            ...
            <Text selectable accessibilityLabel={`小隐寺授权验证码 ${pending.userCode}`} style={{ color: theme.ink, fontSize: 28, fontWeight: '800', letterSpacing: 3 }}>
              {pending.userCode}
            </Text>
      ...
      {phase === 'cleanup' ? (
        ...
        <AppButton
          label={phase === 'expired' ? '重新授权' : phase === 'unsupported' ? '重新检测' : phase === 'error' ? '重试授权' : '授权登录'}
          variant="primary"
          styles={styles}
          onPress={onBegin}
        />
