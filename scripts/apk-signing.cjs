...
function singleApkSignerSha256(output) {
  const numbered = [...String(output || '').matchAll(/Signer #\d+ certificate SHA-256 digest:\s*([a-fA-F0-9:]+)/g)].map(
    (match) => cleanSha256(match[1])
  );
  if (numbered.length) {
    return numbered.length === 1 ? numbered[0] : '';
  }
  const legacy = [
    ...String(output || '').matchAll(/V\d+(?:\.\d+)? Signer: certificate SHA-256 digest:\s*([a-fA-F0-9:]+)/g)
