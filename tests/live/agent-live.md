...
先选择[Profile](#profile)并核对[全局边界](#全局边界)，然后按受影响入口执行：

- [本机导航](#本机导航)、[动态读取与返回](#动态读取与返回)。
- [账号与验证](#账号验证与签到)、[本机与系统能力](#本机与系统能力)。
- [站点互动与上传](#站点互动与上传)须满足本文的[逐次授权门禁](#逐次授权门禁)。
- 更新下载的本机受控入口见 [LOCAL-UPDATE-01](#local-update-01-更新包复用与断点续传)，证据与[报告门禁](#报告与门禁)分别结算。
...
执行顺序固定为：选择能力 ID → 按测试标准完成自动门禁 → 相关 `.ad` Replay → Agent Live 非远端写入场景 → 汇报已完成结果并请求本次远端写入授权 → 只执行用户明确同意的场景。`full` 先运行 `npm run verify`，`targeted` 按受影响入口选择最低可靠自动证据；前一层失败时先记录失败；除非继续操作会造成数据、安全或状态污染，否则仍可收集其他独立场景的证据。
...
| linux.do × 阅读统计、已读与跨端续读 | `LIVE-READ-09` |
...
- 在真实 Feed 录屏执行慢拖、快甩、途中回拖取消、连续反向、相邻和跨页点选。逐帧核对蓝标位置/宽度和文字颜色在手势中连续变化，二级栏随所属 scene 横移且分类/排序属于该站；结束无第二次跳换。点选立即切换，无额外点击动画；取消保持原来源和列表，完成切换只读取最终来源一次并从首项开始。
  - 将列表滚到中部，向右短横滑后在页面尚未归位时立即纵滚，再反向重复；不能等待横向动画结束后才开始下一次拖动。结束时必须呈现一个完整页面，纵向列表确实移动，下一次快甩仍有惯性。用 runbook 的连续触摸矩阵覆盖快中慢、双向和取消；不要用普通单次横滑或外部注入 CANCEL 的通过结果代替自然交接。
  - 分别在静止、纵向惯性刚开始、短横滑接纵滚松手后执行同样的短快滑，左右都必须切换来源；另核对短慢拖自然结算、轻点只停止惯性而不打开帖子。大幅拖动通过不能代替短快滑，完整页面不能代替实际切页断言。
- 尾部手势固定在首页“全部 → 已读”的有限列表验证，确认“已经到底了”后再做双向斜滑；不在未筛选的来源里持续触发无限加载，也不通过打开未读帖增加测试数据。中段与惯性仍用有足够条目的长列表。
- 交叉手势另验：刷新中横滑切来源、下拉中转为横向移动、短拉松手、超长下拉及回拉松手、系统 CANCEL 后下一次刷新；快甩刚松手立即点远端 Tab、首尾页向外滑、二级分类栏横滑、刷新时切底栏再返回。每次核对最终来源、二级栏与结果归属，取消不能残留圆圈或让迟到事件在下一次手势才提交；二级分类通过点击选择，横滑必须露出隐藏分类并能反向滚回，在分类首尾继续横滑也不得切换一级来源；露出的分类必须可实际点击，正文横滑仍切来源。共享 RN RefreshControl 的通知列表需独立核对取消分发和再次刷新，不打开未读消息或执行标已读。
...
### LIVE-READ-09 linux.do 阅读统计与自动续读

- 能力：`TOPIC-01/03`、`NAV-02/03`、`FEED-03`，共享 Search、Library、User、通知话题入口及 `ACCOUNT-04`。使用当前候选 APK 和已确认登录的同一个模拟器；保留安装数据，不清历史、切号或打开未读私信制造样本。此场景包含原站阅读登记，按当次任务已授权的阅读验收范围执行。
- 先通过现有账号等级入口取得同一账号官方 Connect 的阅读前周期指标，并记录来源、时间、指标标签、当前值和要求值；本机估算不能作为官方基线，界面“较上次”也不能替代本轮前后记录。基线之后只在 App 正常阅读选定的普通话题主楼和可见回复，不同时在网页看帖，不手工调用 timings 或额外 POST。返回普通列表后标题立即保持已读、列表位置不变，脱敏请求证据证明返回没有新增列表、详情或阅读状态 GET。重新进入只有一次访问登记；冷加载合并详情、缓存进入不重新加工全文。
- 在与统计对照分开的续读验收轮，先读较高楼层，再向上回看并停留，返回重进须恢复较低楼层及有效内容块。通过 App 内原网页读到更远位置，再新进入 App，应在首次决策窗口采用新进度；通知/明确楼层目标优先于续读，“回到主楼”复用原页面。窗口上下加载、删除隐式目标、同一 route 返回独立核对；没有动态样本的分支不冒充通过。
- 分别从 Search、Library 收藏与历史、User 主题列表进入并返回，核对已读投影、关键词/标签/分页与位置保留，不反查或返回重试批读；聚合“未读”条目点开后退出结果，不请求补页。通知指定楼层优先，不猜测全局未读数；缺少合适通知或收藏样本时单列未验证。
- 本次阅读修复验收范围（2026-09-12 用户确认）：通知返回触发刷新属于原有链路，本轮不修改，也不作为本次关闭条件；上述普通列表返回请求预算不用于将该既有刷新判成本轮失败。通知指定楼层仍必须核对目标身份与定位优先级。此范围说明不改写全局通知产品契约。
- 核对真实可见楼层、图片预览、编辑遮挡及转后台的采集边界；客户端不累计后台时间或跳过的楼层。按 `requestId` 关联 timings 的表单摘要、逐楼毫秒、Native HTTP 状态与单次消费响应后的空体/字节数；话题 GET 核对三项访问参数布尔。缺少对应诊断字段时只能报告已取得的证据，HTTP 成功不能替代字段、空体或统计验收。
- 阅读尾批结算后，通过现有“刷新等级”进行必要的官方 Connect 前后对照，记录实际观测窗口和周期“浏览话题”“浏览帖子”差值；它们不是用户终身累计总量，周期长度只按本次官方明确显示的口径记录，不由要求值推断。Connect 可能返回缓存，当前部署 TTL 未验证，不猜固定等待时间、不连续刷新或加随机 query 刷缓存；短窗口无变化记该轴 `NOT_VERIFIED`，有明确限流时沿 `LIVE-ACCOUNT-04` 处理。网页跨端续读另起一轮，不混入 App-only 统计窗口。
- 累计用户详情或 summary 作为独立统计轴，记录端点类型、前后值和新鲜度证据，不能拿 Connect 的周期值与其相减。原站用户总阅读时长还按相邻 timings 抵达间隔计算，不能要求它精确等于前台秒数；只进行必要的统计读取，不在原网页打开话题制造增量。
- 上游用户统计汇总接口按用户、查看者与语言等在服务端缓存一小时，缓存键不含阅读进度或请求 query；重复刷新或加时间戳不能证明统计新鲜。短窗口总量不变既不判上报失败，也不判统计通过；优先独立核对逐楼 `read`、`last_read_post_number` 和交互，总量新鲜度未确认时该轴记 `NOT_VERIFIED`。重复阅读旧楼不要求已读帖子数增加；总量比较与网页跨端续读分轮进行，避免网页阅读混入 App 增量。依据为 [UsersController](https://github.com/discourse/discourse/blob/c89b1a0506a3ec0a249b7f23ac86763b358dc177/app/controllers/users_controller.rb) 与 [UserStat](https://github.com/discourse/discourse/blob/c89b1a0506a3ec0a249b7f23ac86763b358dc177/app/models/user_stat.rb)。
- 缺少阅读字段的列表本次加载只按最多 50 个 ID 批量补充，显式带 `per_page`，核实当前部署按 ID 返回；缺失 ID 保持未知。部署不支持、限流或验证失败分别报告，不逐帖补请求、不在返回时重试。老帖再次出现时标题已读与新回复提示分别核对。
- 结束恢复首页、原顺序/筛选与前台状态；阅读事实本身不回滚。统计、续读、各入口反馈、请求预算及后台暂停分别报告 `LIVE_PASS`、`NOT_VERIFIED` 或 `BLOCKED_BY_ENV`，不扩大到未测账号切换或私信。
...
- NodeSeek、linux.do 的富文本另核对多张长图：进入图前、两图间和图后时能用普通文字光标输入；上传结束及图片晚到撑高后，光标仍在最后插入图片的紧邻下方并可见。正文中间插图不得跳过后续旧图；手动滚动或移动选区后，晚到图片不得把视口拉回。源码往返、撤销与同实例全屏不得吞图或累积空段落。已有图片回复可只进入本地编辑态检查光标，取消退出，不提交编辑；这部分不代替真实上传验收。
- 长图回复取消再进入，以及退出帖子后重新进入编辑：正文和图片顺序完整。图片慢到时应显示加载占位而非整块空白，失败后可只重试该图；源码始终保留原图地址，重试不重置文字或产生可撤销的正文修改。仅针对当前编辑器图片请求的受控延迟/失败可以证明显示状态，不代表原站网络故障已消除；结束时移除干预，不清全局缓存。
