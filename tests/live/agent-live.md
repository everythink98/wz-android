...
- 交叉手势另验：刷新中横滑切来源、下拉中转为横向移动、短拉松手、超长下拉及回拉松手、系统 CANCEL 后下一次刷新；快甩刚松手立即点远端 Tab、首尾页向外滑、二级分类栏横滑、刷新时切底栏再返回。每次核对最终来源、二级栏与结果归属，取消不能残留圆圈或让迟到事件在下一次手势才提交；二级栏横滑不得意外切来源。共享 RN RefreshControl 的通知列表需独立核对取消分发和再次刷新，不打开未读消息或执行标已读。
