...
- 能力：`TOPIC-02`；共享 `REG-TOPIC-045/046/050/075/095`、`REG-PERF-010`。仅在当前 revision、version/versionCode、APK SHA 与主 AVD 身份匹配时执行；先确认小隐寺 Search 可用且目标标题仍精确为“有关SAAS和储存的几张图”，否则记 `NOT_VERIFIED`，不得换成千图帖子或相似目标。
- 从 Search 选择小隐寺并查询 `SAAS`，打开目标主题和首图，执行慢速拖动过/不过 `18%` 阈值、快速左右甩动、首末边界、两指平移、横向不对称 transform、放大后的单指平移、pinch 复位、1 倍下拉关闭和 Android Back。以 30fps 或 60fps 录屏补充连续同向/方向交替、单指拖动途中加入第二指、双击复位和 portrait/landscape 尺寸变化。每一步按页码确认 index，只使用三张图，不点击保存；坐标手势只属于本监督式 Live，不保存为 tracked Replay。
- oracle：每次有效横滑只结算相邻一页，未过线回弹，首末不循环；pinch、半途加指和 scale 大于 1 的平移全程不改 index。逐帧检查目标图进入中心直到静止的完整窗口，不能在目标之后闪出原图或 `i±2`，也不能出现黑帧；目标/当前 Native media owner 由自动测试固定，设备只补充真实 RNGH/ResumableZoom/Fabric 竞争证据。改变方向或屏幕尺寸后仍满足同一行为。
- 本场景依赖动态第三方内容，不由 `npm run test:device` 或 Smoke 自动执行。三图结果不能证明 2000 项设备 PSS；本轮未执行 25/100 页长列表采样时，`REG-TOPIC-075` 的完整 `LIVE_PASS` 必须记 `NOT_VERIFIED`。
