...
- 受门禁操作包括 NodeSeek 签到、点赞、鸡腿、原站收藏和投票，linux.do 点赞、书签和投票，妖火原站收藏和投票，小隐寺点赞、书签、投票、编辑和删除，以及四站上传图片到回复草稿。上传即使不发送也可能产生远端文件，仍按远端写入处理。
...
- 逐站切换 V2EX、linux.do、NodeSeek、妖火、小隐寺，等待对应列表 ready；各打开一个真实主题。
...
- 普通搜索按受影响来源执行；没有给定 URL 而需要投票目标时，必须在对应来源搜索准确关键词“投票”，打开实际显示投票选项和提交控件的 Topic。给定 URL 时先直达该 Topic，搜索不能替代。
...
- linux.do、小隐寺等级和 NodeImage 只验证当前 App 登录/授权态可见数据与入口，不伪造授权成功；小隐寺不得读取浏览器或 linux.do Cookie。
...
- 只在用户已亲自完成 Device Code 授权，且逐项取得本次真实写操作授权后执行；点赞、原站书签按初始状态恢复，编辑与删除只选择 Agent 本轮获准创建且能安全清理的回复。
