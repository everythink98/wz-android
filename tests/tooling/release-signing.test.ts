...
describe('Android release signing preflight', () => {
  it('accepts exactly one current signer and rejects a second signer', () => {
    ...
    expect(
      singleApkSignerSha256(
        [`Signer #1 certificate SHA-256 digest: ${official}`, `Signer #2 certificate SHA-256 digest: ${other}`].join(
          '\n'
        )
      )
    ).toBe('');
  ...
  it('rejects a missing repository-relative keystore', () => {
    ...
    expect(() => resolveReleaseKeystorePath(project, 'signing/release.jks')).toThrow(
      `keystore 不存在：${path.join(project, 'signing', 'release.jks')}`
    );
  });
  ...
  it('rejects a directory used as the release keystore', () => {
    ...
    expect(() => resolveReleaseKeystorePath(project, 'signing')).toThrow('keystore 不是普通文件');
  });
});
